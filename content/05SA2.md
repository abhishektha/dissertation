...
([Fig. @fig:coefs]) shows the chosen model's normalized regression coefficient values for each shape mode. 
...
All data from this project is available online [@Boppana2021].
...
This was positively affected by foot length and negatively by MTP dorsi/plantarflexion ([@fig:coefs]), suggesting that the instep height increases with foot length, and decreases through heel-off as the MTP dorsiflexes. 
...
At heel-off, there is foot eversion and MTP dorsiflexion [@Leardini2007], explaining the negative effects on the third and fifth shape modes, respectively ([@fig:coefs]).

Girth changes at the ankle, midfoot, and medial MTP joints are captured in the second and sixth shape modes ([@fig:pca_quad]). 
Girths increase just prior toe-off with ankle internal rotation in the sixth shape mode, and eversion in the second and sixth shape modes ([@fig:coefs]).
...
Lasts are typically designed to shape the shoe for both fit and fashion, and are derived from static foot dimensions for a given size [@Luximon2021]. 
...
Lasts could also feature a swappable heel element to ensure that rearfoot morphology is accomodated as the heel is off-loaded. 
...
For example, the instep height drop may be accomodated with tensioned lacing, ensuring the foot is captured and cannot shift upward inside the shoe during heel-off. 
