...
%DIF DEL Boppana_Dissertation_Draft01.00.00.tex   Mon Feb 14 02:05:47 2022
%DIF ADD Boppana_Thesis2.tex                      Mon Feb 28 07:56:43 2022
%DIF 2-4d2
%DIF < %DIF LATEXDIFF DIFFERENCE FILE
%DIF < %DIF DEL Boppana_ComprehensiveExam-Final.tex   Thu Jan  6 08:42:06 2022
%DIF < %DIF ADD Boppana_ComprehensiveExam.tex         Tue Feb  8 00:39:19 2022
%DIF -------
...
%DIFDELCMD < \chapter{Background}\label{background}}
%DIFDELCMD < %%%
...
\DIFaddbegin \hypertarget{sec:SA1}{%
...
%DIF < DIFDELCMD < \begin{figure}
%DIF < DIFDELCMD < \centering
%DIF < DIFDELCMD < %%%
%DIFDELCMD < \DIFdelendFL \DIFaddbeginFL \begin{pandoccrossrefsubfigures}
...
%DIFDELCMD < \caption[{Zoomed-in view of one example step's foot-flat phase showing ``negative'' {[}A{]} and ``positive'' heel-off lag {[}B{]}. The shaded regions represent the detected foot-flat regions for the operator (blue) and spacesuit(red); an increase in vertical acceleration for the shank IMU (blue) prior to the SLL IMU (red) is ``positive'' heel-off lag, suggesting heel-lift {[}B{]}, and vice-versa {[}A{]}.}]{%
{%DIFAUXCMD
%DIFDELCMD < \DIFaddendFL %%%
\DIFdel{Zoomed-in view of one example step's foot-flat phase showing ``negative'' }%DIFDELCMD < {[}%%%
\DIFdel{A}%DIFDELCMD < {]} %%%
\DIFdel{and ``positive'' heel-off lag }%DIFDELCMD < {[}%%%
\DIFdel{B}%DIFDELCMD < {]}%%%
\DIFdel{. The shaded regions represent the detected foot-flat regions for the operator (blue) and spacesuit(red); an increase in vertical acceleration for the shank IMU (blue) prior to the SLL IMU (red) is ``positive'' heel-off lag, suggesting heel-lift }%DIFDELCMD < {[}%%%
\DIFdel{B}%DIFDELCMD < {]}%%%
\DIFdel{, and vice-versa }%DIFDELCMD < {[}%%%
\DIFdel{A}%DIFDELCMD < {]}%%%
\DIFdel{.}}
%DIFAUXCMD
%DIFDELCMD < 
...
%DIF < DIFDELCMD < %%%
%DIFDELCMD < \DIFdelend \DIFaddbegin \end{pandoccrossrefsubfigures}
%DIFDELCMD < \DIFaddend 
%DIFDELCMD < 
...
\DIFdelbegin %DIFDELCMD < \hypertarget{specific-aim-2-predictively-model-dynamic-changes-in-foot-morphology-during-gait}{%
%DIFDELCMD < \chapter{Specific Aim 2: Predictively model dynamic changes in foot morphology during gait}\label{specific-aim-2-predictively-model-dynamic-changes-in-foot-morphology-during-gait}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{sec:SA2}{%
...
%DIF < DIFDELCMD < %%%
%DIFDELCMD < \DIFdelendFL \DIFaddbeginFL %%%
\DIFdelend \begin{pandoccrossrefsubfigures}
\DIFdelbegin %DIFDELCMD < \DIFaddendFL 
%DIFDELCMD < %%%
\DIFdelend 
...
\DIFaddend \subfloat[B]{\includegraphics[width=0.95\textwidth,height=\textheight]{../fig/SA2/PCVAR.png}\label{fig:pca_overlay}}

\DIFdelbegin %DIFDELCMD < \DIFdelbeginFL %%%
%DIF < DIFDELCMD < \caption{%%%
%DIFDELCMD < \DIFdelendFL \DIFaddbeginFL %%%
\DIFdelend \caption[{{[}A{]} Each shape mode's principal axis represented as a heatmap overlaid on the mean foot and shown from 4 different point-of-views. The darker regions represent vertices which are most correlated with the shape mode's principal axis, and therefore see deformations in the shape mode. {[}B{]} Foot shape deformation at +2 and -2 standard deviations along each shape mode's principal axis, overlaid on the mean foot. The point-of-view is set to highlight the major variance along each shape mode's axis.}]{\DIFdelbegin %DIFDELCMD < \DIFaddendFL %%%
\DIFdelend {[}A{]} Each shape mode's principal axis represented as a heatmap overlaid on the mean foot and shown from 4 different point-of-views. The darker regions represent vertices which are most correlated with the shape mode's principal axis, and therefore see deformations in the shape mode. {[}B{]} Foot shape deformation at +2 and -2 standard deviations along each shape mode's principal axis, overlaid on the mean foot. The point-of-view is set to highlight the major variance along each shape mode's axis.}
...
\DIFdelend \end{pandoccrossrefsubfigures}
...
\DIFdelbegin %DIFDELCMD < \hypertarget{introduction-2}{%
%DIFDELCMD < \section{Introduction}\label{introduction-2}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Previously designed planetary spacesuit boots have modified terrestrial hiking boots by integrated a pressure bladder, to allow for interfacing with a pressurized spacesuit.
These boots were designed through iteration and subjective feedback from wearers.
Upon rigorous performance evaluation \mbox{%DIFAUXCMD
\citep{Ross2002, Fineman2018}}\hspace{0pt}%DIFAUXCMD
, though, these designs to date have failed to solve the heel-lift problem, necessitating a new approach to boot design.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \DIFaddbegin \DIFadd{Previously designed planetary spacesuit boots have modified terrestrial hiking boots and integrated a pressure bladder to be pressurized.
%DIFDELCMD < These boots were designed through iteration and subjective feedback.
%DIFDELCMD < To date, these designs have failed to solve the heel-lift problem, necessitating a new approach to boot design.
%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < \DIFaddend %%%
\DIFdelend The design for any new spacesuit component should aim to match the required operator motions for the intended actions, \DIFdelbegin \DIFdel{as well as }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend be sized for the intended population.
This allows for the component to provide proper fit and mobility to the wearer\DIFdelbegin \DIFdel{, but }%DIFDELCMD < \DIFdelbegin \DIFdel{proper design }\DIFdelend \DIFaddbegin \DIFadd{designing such a segment }\DIFaddend %%%
\DIFdelend \DIFaddbegin \DIFadd{.
Designing such a component, however, }\DIFaddend requires an understanding of body segment size and mobility.
\DIFdelbegin %DIFDELCMD < \DIFdelbegin \DIFdel{Planetary spacesuit boots have previously started out with modifying terrestrial hiking boot designs to be pressurized, and then designed through iteration and subjective feedback.
%DIFDELCMD < To date, these designs have failed to solve the heel-lift problem, necessitating a new approach to boot design.
%DIFDELCMD < }\DIFdelend %%%
...
However, there \DIFdelbegin %DIFDELCMD < \DIFdelbegin \DIFdel{is not a clear }\DIFdelend \DIFaddbegin \DIFadd{does not exist a defined }\DIFaddend %%%
\DIFdel{process for integrating }\DIFdelend \DIFaddbegin \DIFadd{does not exist a defined process to design spacesuit components that integrates both traditional and novel data to achieve this objective.
The design process must therefore be modified to integrate }\DIFaddend all available data to \DIFdelbegin \DIFdel{drive spacesuit component design with a focus on improved fit}\DIFdelend \DIFaddbegin \DIFadd{improve spacesuit fit, comfort, }\DIFaddend and mobility.
\DIFdelbegin \DIFdel{This }%DIFDELCMD < \DIFdelbegin \DIFdel{object }\DIFdelend \DIFaddbegin \DIFadd{objective }\DIFaddend %%%
\DIFdel{aims to define that process specifically for the spacesuit boot, through the following objectives:
}\DIFdelend 
...
%DIFDELCMD < \item
%DIFDELCMD <   %%%
\DIFdel{Literature }\DIFdelend \DIFaddbegin \DIFadd{This chapter presents a literature }\DIFaddend review of existing foot shape and mobility \DIFdelbegin \DIFdel{knowledge
}%DIFDELCMD < \item
%DIFDELCMD <   %%%
\DIFdel{Development of }\DIFdelend \DIFaddbegin \DIFadd{data as it relates to spacesuit boot design.
The data is then integrated into }\DIFaddend a design framework\DIFaddbegin \DIFadd{, which is then used }\DIFaddend to design a \DIFdelbegin \DIFdel{more compatible spacesuit boot }%DIFDELCMD < \item
%DIFDELCMD <   %%%
\DIFdel{Design and construction of a spacesuit boot prototype leveraging the design framework
}%DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{novel spacesuit boot with improved fit, comfort, and mobility.
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \hypertarget{fig:SA3-Foot}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.4\textwidth,height=\textheight]{../fig/SA3/FootOverview.png}
%DIFDELCMD < \caption{Foot-specific measures which directly affect mobility and comfort}\label{fig:SA3-Foot}
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\DIFdelbegin \DIFdel{Foot length and width are directly correlated to US shoe sizes for both width and length.
}\DIFdelend \DIFaddbegin \DIFadd{Length is the primary measure used when purchasing shoes, followed by shoe width \mbox{%DIFAUXCMD
\citep{Luximon2013}}\hspace{0pt}%DIFAUXCMD
.
In the US sizing system, each half-size corresponds to a 4.23 cm step.
}\DIFaddend Since females generally feature smaller \DIFaddbegin \DIFadd{length }\DIFaddend feet than males, female shoe size is typically 1.5 \DIFdelbegin \DIFdel{units less than the calculated male size }\DIFdelend \DIFaddbegin \DIFadd{US shoe sizes more than an equivalent male size \mbox{%DIFAUXCMD
\citep{Luximon2013}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
Figure \ref{fig:SA3-ANSUR} shows that this offset does not sufficiently align the female population to the male population \DIFaddbegin \DIFadd{for shoe length.
Shoe width sizes are graded with letters (``AA'' to ``EE'') in the US, offset from a normal ``D'' in steps of 6.35 mm; a ``C'' is 6.35mm narrower while a ``E'' is 6.35mm wider \mbox{%DIFAUXCMD
\citep{Luximon2013}}\hspace{0pt}%DIFAUXCMD
.
Shoe width sizes also scale with length; as a longer foot is typically wider \mbox{%DIFAUXCMD
\citep{Jurca2019}}\hspace{0pt}%DIFAUXCMD
.
...
Therefore, when designing a shoe, it is important to consider the target audience's foot anthropometry outside of the shoe sizing standard and use foot measurements when available}\DIFaddend .
\DIFdelbegin \DIFdel{Therefore, it is important to use foot lengthas a direct measure when fitting or selecting a shoeas opposed to shoe size.
}\DIFdelend 

Arch length denotes the location of the metatarsophalangeal (MTP) joints on the foot\DIFdelbegin \DIFdel{, one of the important joints during gait}\DIFdelend .
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \hypertarget{fig:SA3-ANSUR}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.8\textwidth,height=\textheight]{../fig/SA3/ANSUR.png}
%DIFDELCMD < \caption{(Right) Inequality in distribution of equivalent shoe size between male and female, (Left) relationship between foot length and arch length; visualizations developed from the ANSUR II Dataset}\label{fig:SA3-ANSUR}
%DIFDELCMD < }
%DIFDELCMD < %%%
...
Voloshina et al. \citep{Voloshina2013} found that during gait on uneven surfaces, the ankle does not flex past \DIFdelbegin \DIFdel{+/- }\DIFdelend \DIFaddbegin \DIFadd{\(\pm\) }\DIFaddend 20 degrees.
...
The proposed design framework will link \DIFaddbegin \DIFadd{traditional linear }\DIFaddend foot measurements described in the previous section and the dynamic foot shape model \DIFaddbegin \DIFadd{(sec.~\ref{sec:SA2}) }\DIFaddend to specific footwear design variables, allowing for the design of a spacesuit boot with \DIFdelbegin \DIFdel{proper fit}\DIFdelend \DIFaddbegin \DIFadd{an emphasis on improving fit, comfort, }\DIFaddend and mobility.
\DIFdelbegin \DIFdel{The framework assumes the development of }\DIFdelend \DIFaddbegin \DIFadd{Note, the framework assumes }\DIFaddend a gas-pressurized spacesuit boot to maintain compatibility with the current xEMU architecture.
...
\DIFdelbegin \DIFdel{Footwear design variablesare }\DIFdelend \DIFaddbegin \DIFadd{The framework incorporates two types of design variables,}\DIFaddend categorized as either population measures or individual measures.
...
\DIFaddbegin \DIFadd{Angular and morphology measurements can inform footwear design for population measurements.
These measurements aim to accommodate the shape of the foot, as well as its mobility for the intended activity.
}

\DIFaddend Individual design variables \DIFdelbegin \DIFdel{will be sizing }\DIFdelend \DIFaddbegin \DIFadd{are used to size }\DIFaddend specific elements which are \DIFdelbegin \DIFdel{changed }\DIFdelend \DIFaddbegin \DIFadd{scaled }\DIFaddend between sets of boots to fit inter-individual differences\DIFdelbegin \DIFdel{(such as shoe size).
...
\DIFaddend Figure \ref{fig:SA3-Overview} shows how each of these measures is mapped to footwear design variables.
\DIFaddbegin \DIFadd{In the following sections, the design requirements specific to a planetary spacesuit boot are outlined.
}\DIFaddend 
...
The proposed design framework targets higher flexion/extension capability in the ankle joint \DIFdelbegin \DIFdel{, as well as specifies }\DIFdelend \DIFaddbegin \DIFadd{to match ankle mobility.
The framework also specifies a requirement for }\DIFaddend extension of the MTP joint, limited ankle internal/external rotation, and limited ankle inversion/eversion.
...
The \DIFdelbegin %DIFDELCMD < \DIFdelbegin \DIFdel{dynamic foot shape model drive the design of an upper which can be easily scaled to different shoe sizes.
%DIFDELCMD < }%%%
%DIF < DIFDELCMD < 
\DIFdelend \DIFaddbegin \DIFadd{instep height measurement is of particular importance, as it represents the height of the foot from the plantar surface to the dorsal surface just in front of the ankle joint.
...
%DIF < DIFDELCMD < %%%
\DIFdelbegin %DIFDELCMD < \DIFdel{The upper's location between the MTP and ankle joint, and its requirement to conform to the shape }\DIFdelend \DIFaddbegin \DIFadd{instep height measurement is of particular importance, as it represents the height }\DIFaddend %%%
\DIFdel{of the foot }%DIFDELCMD < \DIFdelbegin \DIFdel{, drive the selection of a softer, flexible fabric being used to meet these requirements.
%DIFDELCMD < This presents a challenge with designing the pressure bladder, as the pressure bladder is inherently stiff under pressure.
%DIFDELCMD < Therefore, a soft inner layer above the dorsum may be used which allows the stiff pressurized bladder to conform to the individual's dorsum.
%DIFDELCMD < Since the dorsum still transmits power to push the shoe off the ground, the soft layer still needs to have enough structure to transmit this power.
%DIFDELCMD < If too soft, the layer will simply act as empty space and the shoe will not respond to ankle flexion during heel-off, potentially resulting in heel-lift.
%DIFDELCMD < }\DIFdelend \DIFaddbegin \DIFadd{from the plantar surface to the dorsal surface just in front of the ankle joint.
%DIFDELCMD < The instep height measurement was found to have both static and dynamic variability from the dynamic foot shape model in Chapter 5.
%DIFDELCMD < Therefore, the upper should accommodate this variability to ensure fit can be maintained through the gait cycle for a wide range of static instep heights.
%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < \DIFaddend %%%
\DIFdelend Lacing or other closure mechanisms \DIFdelbegin %DIFDELCMD < \DIFdelbegin \DIFdel{would further allow the shoe upper to conform to the dorsum and capture the foot.
%DIFDELCMD < Furthermore,
%DIFDELCMD < the closure mechanism should be customizable }\DIFdelend \DIFaddbegin \DIFadd{can assist the upper in conforming to and capturing the foot,
%DIFDELCMD < Closure mechanisms allow for customization }\DIFaddend %%%
\DIFdelend \DIFaddbegin \DIFadd{can assist the upper in conforming to and capturing the foot (sec.~\ref{sec:SA2}).
Closure mechanisms allow for customization }\DIFaddend by the individual wearing the boot, so each wearer can adjust \DIFdelbegin %DIFDELCMD < \DIFdelbegin \DIFdel{to where they feel is comfortable}\DIFdelend \DIFaddbegin \DIFadd{the boot to their preferred fit}\DIFaddend %%%
\DIFdelend \DIFaddbegin \DIFadd{the boot to their preferred fit}\DIFaddend .
Conforming the upper to the \DIFdelbegin %DIFDELCMD < \DIFdelbegin \DIFdel{dorsum }\DIFdelend \DIFaddbegin \DIFadd{foot }\DIFaddend %%%
\DIFdelend \DIFaddbegin \DIFadd{foot }\DIFaddend will also eliminate any empty space between the foot and above the dorsum, reducing the chance of heel-lift since the foot will no longer be allowed to move within the boot.
In addition, this reduces the chance of contact injuries from rubbing between the foot and boot.

The upper \DIFdelbegin %DIFDELCMD < \DIFaddbegin \DIFadd{and lacing }\DIFaddend %%%
\DIFdelend \DIFaddbegin \DIFadd{and lacing }\DIFaddend will also play a role in donning and doffing of the spacesuit boot.
...
The closure \DIFdelbegin %DIFDELCMD < \DIFdelbegin \DIFdel{can }\DIFdelend \DIFaddbegin \DIFadd{will need to }\DIFaddend %%%
\DIFdelend \DIFaddbegin \DIFadd{will need to }\DIFaddend be designed in conjunction with \DIFdelbegin %DIFDELCMD < \DIFdelbegin \DIFdel{a single structured fold in the pressure bladderto allow }\DIFdelend \DIFaddbegin \DIFadd{pressure bladder, as }\DIFaddend %%%
\DIFdelend \DIFaddbegin \DIFadd{pressure bladder, as }\DIFaddend the pressure bladder \DIFdelbegin %DIFDELCMD < \DIFdelbegin \DIFdel{to change shape }\DIFdelend \DIFaddbegin \DIFadd{will need to deform in a structured matter when the laces are tightened.
%DIFDELCMD < The boot can then loosen }\DIFaddend %%%
\DIFdelend \DIFaddbegin \DIFadd{will need to deform in a structured matter when the laces are tightened.
The boot can then loosen }\DIFaddend and allow the foot to be \DIFdelbegin %DIFDELCMD < \DIFdelbegin \DIFdel{released from the boot.
%DIFDELCMD < Figure \ref{fig:SA3-Upper} shows a possible configuration of the upperusing laces which conforms to }\DIFdelend \DIFaddbegin \DIFadd{removed.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{removed.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \DIFadd{The upper's location between the MTP and ankle joint, and its requirement to conform to }\DIFaddend %%%
\DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{The upper's location between the MTP and ankle joint, and its requirement to conform to the }\DIFaddend shape of the foot\DIFdelbegin %DIFDELCMD < \DIFdelbegin \DIFdel{while still allowing for donning and doffing}\DIFdelend \DIFaddbegin \DIFadd{, may drive the selection of a softer, flexible fabric being used to meet these requirements.
%DIFDELCMD < This presents a challenge with designing the pressure bladder, as the pressure bladder is inherently stiff under pressure.
%DIFDELCMD < Therefore, a soft inner layer above the dorsum may be used which allows the stiff pressurized bladder to conform to the individual's dorsum.
%DIFDELCMD < Since the dorsum still transmits power to push the shoe off the ground, the soft layer still needs to have enough structure to transmit this power.
%DIFDELCMD < If too soft, the layer will simply act as empty space and the shoe will not respond to ankle flexion during heel-off, potentially resulting in heel-lift}\DIFaddend %%%
\DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, may drive the selection of a softer, flexible fabric being used to meet these requirements.
...
\DIFdelbegin %DIFDELCMD < \DIFdelbegin %%%
%DIF < DIFDELCMD < \begin{figure}
%DIF < DIFDELCMD < \hypertarget{fig:SA3-Upper}{%
%DIF < DIFDELCMD < \centering
%DIF < DIFDELCMD < \includegraphics[width=0.5\textwidth,height=\textheight]{../fig/SA3/Upper.png}
%DIF < DIFDELCMD < \caption{Conceptual design of a boot's upper configuration with pressure bladder}\label{fig:SA3-Upper}
%DIF < DIFDELCMD < }
%DIF < DIFDELCMD < \end{figure}
%DIF < DIFDELCMD < 
%DIFDELCMD < 
...
\DIFdelbegin \DIFdel{Dynamic foot shape data can provide a base contour for the sole to be able to bend at }\DIFdelend \DIFaddbegin \DIFadd{The arch length measurement can be used to ensure }\DIFaddend the MTP joint \DIFdelbegin \DIFdel{during heel-off, as shown in }\DIFdelend \DIFaddbegin \DIFadd{of a wearer is properly indexed to the sole's flexibility, allowing the MTP joint to act as intended (}\DIFaddend fig.~\ref{fig:SA3-SoleFlex}\DIFdelbegin \DIFdel{.
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \hypertarget{fig:SA3-SoleFlex}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.5\textwidth,height=\textheight]{../fig/SA3/SoleFlex.png}
%DIFDELCMD < \caption{Desired sole flexibility (red) matched with plantar foot contour at MTP joint}\label{fig:SA3-SoleFlex}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \hypertarget{fig:SA3-SoleFlex}{%
\centering
...
\DIFdelbegin %DIFDELCMD < \DIFdelbegin %%%
%DIF < DIFDELCMD < \hypertarget{framework-implementation}{%
%DIF < DIFDELCMD < \section{Framework Implementation}\label{framework-implementation}}
%DIF < DIFDELCMD < %%%
%DIFDELCMD < \DIFdelend \DIFaddbegin \hypertarget{prototype-planetary-spacesuit-boot-design}{%
%DIFDELCMD < \section{Prototype Planetary Spacesuit Boot Design}\label{prototype-planetary-spacesuit-boot-design}}
%DIFDELCMD < \DIFaddend 
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{novel-planetary-spacesuit-boot-design}{%
\section{Novel Planetary Spacesuit Boot Design}\label{novel-planetary-spacesuit-boot-design}}

\DIFadd{The biomechanical design framework was used to inform the design of a novel planetary spacesuit boot with improved fit and comfort compared to current prototypes.
...
\DIFdelbegin \DIFdel{The }%DIFDELCMD < \DIFdelbegin \DIFdel{presented framework outlines the integration of novel dynamic foot morphology data into the boot design process.
%DIFDELCMD < This process can help design a novel spacesuit boot that aims to both fit the foots shape and proper foot mobility, reducing the risk of injury and fit issues such as heel-lift.
%DIFDELCMD < Designing a }\DIFdelend \DIFaddbegin \DIFadd{biomechanical design framework was used to inform the design of a prototype planetary spacesuit boot with improved fit and comfort compared to current prototypes.
%DIFDELCMD < The boot is designed to be compatible with gas pressurized spacesuits.
%DIFDELCMD < When a }\DIFaddend %%%
\DIFdel{spacesuit }%DIFDELCMD < \DIFdelbegin \DIFdel{boot design from scratch rather than modifying existing terrestrial boots can ensure better fitand mobility, reducing the risk of injury.
%DIFDELCMD < The resulting boot design from this framework and corresponding computational analysis will serve as a starting point for iteration of a spacesuit bootprototype.
%DIFDELCMD < }%%%
%DIF < DIFDELCMD < 
\DIFdelend \DIFaddbegin \DIFadd{The boot is designed to be compatible with gas pressurized spacesuits.
...
Previous efforts at integrating lacing into the boot only focused on running the lacing through the restraint layer \mbox{%DIFAUXCMD
\citep{Graziosi2016}}\hspace{0pt}%DIFAUXCMD
, and did not have an accompanying conformal pressure bladder.
...
%DIF < DIFDELCMD < %%%
\DIFdelbegin %DIFDELCMD < \DIFdel{The spacesuit boot prototype will require construction with a pressure bladder for pressurization.
%DIFDELCMD < Heat-sealable urethane coated nylon (400D, Seattle Fabrics)will be used for }\DIFdelend \DIFaddbegin \DIFadd{is pressurized, it's pressure bladder expands to a stiff shape with increased volume.
%DIFDELCMD < The stiff structure must be deformed by the wearer to provide mobility, as described in Chapter 2.
%DIFDELCMD < In addition, this structure is the closest layer in a spacesuit assembly to the spacesuit operator, and therefore plays an important role in fit and comfort.
%DIFDELCMD < A spacesuit's pressure bladder is typically paired with a restraint layer to control the expansion of the bladder\mbox{%DIFAUXCMD
%DIFDELCMD < \citep{Harris2001}}\hspace{0pt}%DIFAUXCMD
%DIFDELCMD < .
%DIFDELCMD < However, through discussions with spacesuit engineers, the pressure bladder was found to be the most important layer in providing fit, comfort, and mobility.
%DIFDELCMD < Therefore, }\DIFaddend %%%
\DIFdel{the }%DIFDELCMD < \DIFdelbegin \DIFdel{pressure bladder, which was recommended by spacesuit professionals.
%DIFDELCMD < Fabrication capabilities for the pressure bladder will be iteratively developed, starting with adding a pressure bladder to a terrestrial boot }\DIFdelend \DIFaddbegin \DIFadd{design of the prototype planetary spacesuit boot was restricted to the pressure bladder layer without an accompanying restraint layer.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Some design elements from previous planetary spacesuit boot designs were implemented to meet the biomechanical and anthropometric requirements presented in the framework above.
The following sections describe the design decisions made for various parts of the prototype planetary spacesuit boot.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \DIFadd{The primary goal of this design was to accommodate a wide range of instep heights, due to both static and dynamic variability as described in Chapter 5.
%DIFDELCMD < Design elements from previous planetary spacesuit boot designs were implemented to meet the biomechanical and anthropometric requirements presented in the framework above.
%DIFDELCMD < The following sections describe the design decisions made for various parts of the prototype planetary spacesuit boot}\DIFaddend %%%
\DIFdel{.
}%DIFDELCMD < \DIFdelbegin \DIFdel{As design elements are finalized, they will integrated into prototype iterations.
%DIFDELCMD < When the design is finalized, a new boot will be constructed with all design elements.
%DIFDELCMD < A pressurization level of 3.5 psi will first be targeted, consistent with the operating pressure of the launch-entry-abort suits.
%DIFDELCMD < The final operating pressure will target 4.3 psi, the operating pressure of the EMU spacesuit.
%DIFDELCMD < }\DIFdelend 
%DIFDELCMD < 

%DIFDELCMD < \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin %%%
\DIFdelend \hypertarget{ankle-convolutes}{%
\subsection{Ankle Convolutes}\label{ankle-convolutes}}

\DIFdelbegin %DIFDELCMD < \DIFadd{Current planetary spacesuit prototypes use single-axis convolute assembly with three segments to provide ankle flexion/extension, but not inversion/eversion \mbox{%DIFAUXCMD
%DIFDELCMD < \citep{Ross2002}}\hspace{0pt}%DIFAUXCMD
%DIFDELCMD < .
%DIFDELCMD < A segment consists of a circular piece of fabric; individual segments are assembled with overlap over consecutive segments.
%DIFDELCMD < While a variety of convolute joint designs are available (see Chapter 2), rolling convolutes were chosen for this boot design after prototyping efforts found these to be the most easily constructed in a laboratory setting.
%DIFDELCMD < Rolling convolute joints feature a band around each convolute segment, which guide the convolute's rolling and unrolling during motion \mbox{%DIFAUXCMD
%DIFDELCMD < \citep{Harris2001}}\hspace{0pt}%DIFAUXCMD
%DIFDELCMD < .
%DIFDELCMD < When a rolling convolute ankle joint is flexed, one side of the convolute segments roll over each other and collapse, while the convolute segments on the opposite side unroll and expand fig.~\ref{fig:SA3-convolute_actuation}.
%DIFDELCMD < A longitudinal restraint strap is attached to the convolute segments to ensure that the convolutes do not unfold completely from pressurization fig.~\ref{fig:SA3-convolute_actuation}.
%DIFDELCMD < The rolling convolute joint also does not require a restraint layer to function, allowing it to be integrated within the scope of this design.
%DIFDELCMD < It was decided to retain the number of convolutes from the planetary spacesuit prototypes, but recalculate the sizing and placement of the convolute segments to meet the requirements from the biomechanical boot design framework.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Current planetary spacesuit prototypes use single-axis convolute assembly with three segments to provide ankle flexion/extension, but not inversion/eversion \mbox{%DIFAUXCMD
...
\DIFdelbegin %DIFDELCMD < \DIFadd{Range-of-mobility for a convolute assembly is limited by the number of convolute segments, height of each convolute segment, and the overlap between each segment.
%DIFDELCMD < When a convolute assembly is fully actuated, its segments are fully collapsed on one side, and fully extended on the other side.
%DIFDELCMD < This can be simplified into a triangle, to show the extent to which the convolute assembly is bent fig.~\ref{fig:SA3-convolute_tri}.
%DIFDELCMD < The width of this triangle represents the diameter of the convolute assembly.
%DIFDELCMD < The width and hypotenuse of the triangle are equal as the diameter of assembly is constant for each segment.
%DIFDELCMD < The height of this triangle is the total actuated height of the convolute assembly when it is fully expanded on one side, and collapsed on the opposite side.
%DIFDELCMD < A convolute assembly's expanding side can only unroll until it is no longer overlapping the preceding segment fig.~\ref{fig:SA3-convolute_actuation}.
%DIFDELCMD < Similarly, the collapsing side of the assembly can only roll until each segment reaches the bottom of the succeeding segment.
%DIFDELCMD < The vertex connecting the base and hypotenuse of the simplified triangle represents the collapsing side of the assembly.
%DIFDELCMD < The simplified triangle's height represents the expansion of the assembly; as the convolute assembly has a resting height when it is not actuated, the triangle's height only represents the total overlap between all segments.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Range-of-mobility for a convolute assembly is limited by the number of convolute segments, height of each convolute segment, and the overlap between each segment.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{The total overlap of the convolute segments can therefore be determined by the diameter of the convolute assembly and the desired range-of-motion (ROM) from this triangle:
%DIFDELCMD < \$
%DIFDELCMD < overlap\_\{total\} = diameter*tan(ROM)
%DIFDELCMD < \$
%DIFDELCMD < As unrolling and rolling occur simultaneously for each segment, the placement of a convolute segment should be such that its overlap with preceding and consecutive segments is half the segment height.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The total overlap of the convolute segments can therefore be determined by the diameter of the convolute assembly and the desired range-of-motion (ROM) from this triangle:
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \DIFadd{To determine the individual convolute segment heights, the total overlap is divided by the number of convolute segments minus one, as the total overlap is distributed across all convolute segments.
%DIFDELCMD < In addition, the final convolute segment in the assembly does not have a succeeding segment to roll over.
%DIFDELCMD < The height of a convolute segment should be
%DIFDELCMD < Therefore, the height of a convolute segment given a desired range-of-motion (ROM) is:
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{equation}\DIFadd{\protect\hypertarget{eq:SA3-overlap}{}{
overlap_{total} = diameter*tan(ROM)
}\label{eq:SA3-overlap}}\end{equation}
\DIFaddend 

\DIFaddbegin \DIFadd{As unrolling and rolling occur simultaneously for each segment, the placement of a convolute segment should be such that its overlap with preceding and consecutive segments is half the segment height.
}
...
\DIFaddend \begin{equation}\DIFdelbegin %DIFDELCMD < {
%DIFDELCMD < \DIFadd{height = 2*\frac{diameter * tan(ROM)}{n_{segments}-1}
%DIFDELCMD < }}\DIFadd{}%%%
\DIFdelend \DIFaddbegin \protect\hypertarget{eq:SA3-height}{}{
height = 2*\frac{diameter * tan(ROM)}{n_{segments}-1}
}\label{eq:SA3-height}\DIFaddend \end{equation}

\DIFdelbegin %DIFDELCMD < \DIFadd{Each convolute segment should be placed in the assembly such that it is overlapping the successive segment by one-half of its height.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Each convolute segment should be placed in the assembly such that it is overlapping the successive segment by one-half of its height.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \DIFadd{Circumferential sizing of the ankle convolute assembly was determined by the requirement of the boot to be donned and doffed over the wearer's foot, ankle, and calf.
%DIFDELCMD < Of these, the heel-ankle circumference was identified as the largest dimension around which the ankle convolute assembly needs be donned/doffed over \mbox{%DIFAUXCMD
%DIFDELCMD < \citep{Gordon2014}}\hspace{0pt}%DIFAUXCMD
%DIFDELCMD < .
%DIFDELCMD < The 95\% percentile of male heel-ankle circumference, 372 mm, was chosen as the circumference of the ankle convolutes, allowing the convolute assembly to be donned over the foot.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Circumferential sizing of the ankle convolute assembly was determined by the requirement of the boot to be donned and doffed over the wearer's foot, ankle, and calf.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{The corresponding diameter to a circumference of 372 mm is 184 mm (\(372mm/\pi\)).
%DIFDELCMD < As the desired dorsiflexion ROM (40 degrees) is greater than the desired plantarflexion ROM(20 degrees), this will drive the convolute height calculation.
%DIFDELCMD < Using the above equation, the resulting convolute segment height is 100 mm.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{A circumference of 372 mm has a diameter of 184 mm (\(\frac{372mm}{\pi}\)).
...
\DIFdelbegin %DIFDELCMD < \DIFadd{A longitudinal restraint strap is needed to ensure the convolutes do not fully expand during pressurization, and to balance the expansion and contraction of the convolutes during joint actuation.
%DIFDELCMD < This restraint strap fixes the overlap of the convolute segments at the medial and lateral extents of the joint, allowing the assembly to provide plantar and dorsiflexion.
%DIFDELCMD < The restraint strap should therefore be sized to the height of each convolute segment, times the number of convolute segments.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{A longitudinal restraint strap is needed to ensure the convolutes do not fully expand during pressurization, and to balance the expansion and contraction of the convolutes during joint actuation.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{The ankle joint convolute assembly should also be able to be actuated by the operator's ankle joint, whose joint center is at the malleolus.
%DIFDELCMD < The ankle convolute segments in current planetary spacesuit boots are placed above the instep region, as their vertical height needs to be accommodated
%DIFDELCMD < The joint center of the convolute system is at the center of the middle convolute segment.
%DIFDELCMD < For an ankle joint, this places the joint center well above the malleolus.
%DIFDELCMD < Therefore, the operator's ankle joint is not indexed properly within the spacesuit boot.
%DIFDELCMD < Placing the convolute segments at an angle with respect to the plantar-plane of the foot allows the joint segment to be shifted downward (fig.~\ref{fig:SA3-convolute_ang}).
%DIFDELCMD < A 25-degree tilt was applied to the convolute segments to lower the joint center such that it is closer to the malleolus, improving the indexing of the operator's ankle joint inside the boot.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The ankle joint convolute assembly should also be able to be actuated by the operator's ankle joint, whose joint center is at the malleolus.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{The upper is the part of the boot ventral of the MTP-joint and above the sole.
%DIFDELCMD < The toe-box refers to the part of the boot distal of the MTP-joint.
%DIFDELCMD < The upper of the planetary spacesuit boot prototype should allow the foot to drive the motion of the shoe while providing proper fit and comfort, while the toe-box simply accommodates the toes.
%DIFDELCMD < Shoe uppers are based on a last: a mold representing the foot shape around which the shoe is constructed.
%DIFDELCMD < A last is characterized by its heel-to-toe drop and toe-spring, two sole design features which are very specific to the application of the shoe.
%DIFDELCMD < As a planetary spacesuit boot is tasked with allowing an astronaut to work and ambulate comfortably across a rough and unknown terrain, parameters from a hiking boot can be used to select a last.
%DIFDELCMD < A male size-9 hiking last was provided for this project from the University of Oregon Sports Product Design Program.
%DIFDELCMD < This last features a minimal heel-to-toe drop and a toe-spring of 15 degrees.
%DIFDELCMD < The last features a roomy toe box, where the toe-box is larger than the volume of the toes, for comfort.
%DIFDELCMD < The last was widened at the ankle to interface to a circumference of 372mm, the same dimension as the ankle convolutes.
%DIFDELCMD < This allows the ankle convolute assembly to be attached to the upper.
%DIFDELCMD < The original last and the widening modification are shown in fig.~\ref{fig:SA3-lastmod}.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The upper of the planetary spacesuit boot prototype should allow the foot to drive the motion of the shoe while providing proper fit and comfort, while the toe-box simply accommodates the toes.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{The last was 3D printed with ABS.
%DIFDELCMD < A flat pattern of the last was generated by placing masking tape over the last and cutting along seam lines as shown in Fig }{[}\DIFadd{XX}{]} \DIFadd{\mbox{%DIFAUXCMD
%DIFDELCMD < \citep{Motawi2020}}\hspace{0pt}%DIFAUXCMD
%DIFDELCMD < .
%DIFDELCMD < The cut pieces are then placed on white paper to create patterns which represent the planetary spacesuit }\DIFaddend %%%
\DIFdel{boot's }%DIFDELCMD < \DIFdelbegin \DIFdel{design will use some existing, proven spacesuit construction techniques to ensure success.
%DIFDELCMD < The pressure bladder will be fixed to the inside }\DIFdelend \DIFaddbegin \DIFadd{upper.
%DIFDELCMD < These seams were chosen to ensure the patterns are as flat as possible.
%DIFDELCMD < As a result, separate patterns were generated for the sole and toe box, and the upper was split into multiple patterns.}%%%
\DIFdelend \DIFaddbegin \DIFadd{The standard method to generate flat patterns from a last is to place masking tape on the last, cut along seam lines, and flatten the masking tape patterns \mbox{%DIFAUXCMD
...
As a result, separate patterns were generated for the sole and toe box, and the upper was split into multiple patterns.}\DIFaddend \\
\DIFdelbegin %DIFDELCMD < \DIFadd{The seam which runs along the foot's length across the upper was used to incorporate lacing into the boot.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The seam which runs along the foot's length across the upper was used to incorporate lacing into the boot.
}\DIFaddend 
...
\DIFaddend \hypertarget{instep-lacing}{%
\subsection{Instep Lacing}\label{instep-lacing}}

\DIFdelbegin %DIFDELCMD < \DIFadd{Lacing in a terrestrial shoe allows for adjustment of the shoe's upper to accommodate arch height variability.
%DIFDELCMD < Lacing is typically paired with a tongue, which will raise or lower in height to provide the best fit and comfort to the wearer \mbox{%DIFAUXCMD
%DIFDELCMD < \citep{Feeney2019}}\hspace{0pt}%DIFAUXCMD
%DIFDELCMD < .
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{A rolling convolute was used to make a conformal pressure bladder which accommodates the lacing, and changes its volume as the lacing is tightened.
This convolute allow for deformation of the pressure bladder without resulting in folds, which have been shown to cause discomfort \mbox{%DIFAUXCMD
\citep{Chappell2017}}\hspace{0pt}%DIFAUXCMD
...
\DIFdelbegin %DIFDELCMD < \DIFadd{For lacing to behave similarly in a spacesuit boot, it must be accompanied by a pressure bladder design with a structure that when deformed by the lacing, results in a tighter fit.
%DIFDELCMD < Without such a design, deforming the pressure bladder will result in folds, which have been shown to cause discomfort \mbox{%DIFAUXCMD
%DIFDELCMD < \citep{Chappell2017}}\hspace{0pt}%DIFAUXCMD
%DIFDELCMD < , and may be unpredictable in their deformation.
%DIFDELCMD < A convolute can be used to control the deformation; as the lacing is tightened, it can contract the convolute in a prescribed fashion which will cause the distance between the plantar and dorsal surface }\DIFaddend %%%
\DIFdel{of the shoe}%DIFDELCMD < \DIFdelbegin \DIFdel{using a series of tabs, ensuring that it is always indexed properly inside the foot.
%DIFDELCMD < A convoluted joint will be used for ankle jointmobility, similar to \mbox{%DIFAUXCMD
%DIFDELCMD < \citep{Harris2001}}\hspace{0pt}%DIFAUXCMD
%DIFDELCMD < .
%DIFDELCMD < The upper will act as a pressure restraint layer to prevent the pressure bladder from bellowing once pressurized.
%DIFDELCMD < Collaborations will be established with footwear design experts }\DIFdelend \DIFaddbegin \DIFadd{'s interior to decrease and result in a tighter fit.
%DIFDELCMD < When the lacing is loosened, the convolute is expanded and the volume is increased, resulting in a looser fit.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Initial designs of the lacing convolute had the convolute placed around the entire foot fig.~\ref{fig:SA3-lacingevo}.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{Initial designs of the lacing convolute had the convolute placed around the entire foot fig.~\ref{fig:SA3-lacingevo}.
%DIFDELCMD < Thus, when the convolute is contracted, the distance between the plantar and dorsal surface decreases uniformly across the length of the foot.
%DIFDELCMD < However, findings from the dynamic foot shape model (See Chapter 5) found that instep height variability mostly occurs towards the ankle joint.
%DIFDELCMD < Therefore, the lacing convolute design was modified to pivot at the MTP joint.
%DIFDELCMD < The lacing convolute is designed into the seam between the inferior and superior upper patterns.
%DIFDELCMD < The overlap for the lacing convolute was designed with a varying height across the length of the upper patterns, as opposed to the constant height of the overlap used for the ankle convolutes.
%DIFDELCMD < The overlap is 1 cm at the toe-box, and 5 cm near the heel, allowing for the pivot at the MTP joint.
%DIFDELCMD < }
%DIFDELCMD < 
...
\DIFdelbegin %DIFDELCMD < \DIFadd{The convolute is paired with a lacing system to allow the wearer to adjust the fit of the upper to their desired tightness (}{[}\DIFadd{fig:SA3-lacingoverview}{]}\DIFadd{).
%DIFDELCMD < In a spacesuit boot, the lacing will need to withstand the pressure forces; the pressure forces will want to expand the boot and loosen the lacing.
%DIFDELCMD < A BOA lacing system (BOA, Denver CO) was chosen for its strength and fine adjustment, and was graciously provided by BOA.
%DIFDELCMD < The BOA H4 system consists of a dial, a steel lace, tubing, and low-friction guides; this is the strongest system provided by BOA.
%DIFDELCMD < The guides are attached in an alternating pattern between the top and bottom layers of the convolute.
%DIFDELCMD < The lace is run from the dial, through the tubing on the medial side, through the guides, and back through the tubing on the lateral side to the dial.
%DIFDELCMD < As the dial is tightened, the lacing length is decreased, bringing the guides closer together and contracting the convolute.
%DIFDELCMD < The lace length is kept constant through the tubing, which fixes the length between the dial and the guides.
%DIFDELCMD < This is necessary to ensure that no tightening is occurring near the ankle convolute assembly, which would prevent the ankle convolutes from working.
%DIFDELCMD < Once the lace is tightened to the desired level, the dial can be locked; this prevents the lace from lengthening or shortening.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The convolute is paired with a lacing system to allow the wearer to adjust the fit of the upper to their desired tightness (fig.~\ref{fig:SA3-lacingoverview}).
...
\DIFdelbegin %DIFDELCMD < \DIFadd{The placement of the dial was chosen to provide dynamic tightening (}{[}\DIFadd{fig:SA3-lacingoverview}{]}\DIFadd{).
%DIFDELCMD < The dial is placed on the front of the convolute assembly.
%DIFDELCMD < The tubing is run on the distal side of the convolute assembly.
%DIFDELCMD < Therefore, when the ankle is plantarflexed, such as during heel-off, the distance that the lace travels increases.
%DIFDELCMD < Since the lace length is locked, the guides must move closer together to accommodate the lace travel length; this therefore rolls the convolute and tightens the fit.
%DIFDELCMD < The dynamic tightening accommodates the drop in instep height characterized in the dynamic foot shape model (Chapter 5), ensuring that fit is maintained throughout the gait cycle.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{All lacing adjustment is done when the boot is unpressurized.
...
\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD < \hypertarget{fig:SA3-lacingoverview}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.7\textwidth,height=\textheight]{../fig/SA3/LacingOverview.png}
%DIFDELCMD < \caption{Overview of lacing path on prototype spacesuit boot. Only the medial side is shown, but the lacing is mirrored on the lateral side. When plantarflexed, the distance the lace has to travel is increased and is shown by the arrow. Therefore, the lacing near the guides further tightens.}\label{fig:SA3-lacingoverview}
%DIFDELCMD < }
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The placement of the dial was chosen to provide dynamic tightening (fig.~\ref{fig:SA3-lacingoverview}).
...
\DIFdelbegin %DIFDELCMD < \DIFadd{The heel counter is used in footwear to provide a stiffer rear foot support \mbox{%DIFAUXCMD
%DIFDELCMD < \citep{Van1995}}\hspace{0pt}%DIFAUXCMD
%DIFDELCMD < , which may prevent the rear of the shoe from deforming and allowing the heel to lift out of the shoe.
%DIFDELCMD < The placement of the rearmost BOA dial (fig.~\ref{fig:SA3-lacingoverview}) acts as a heel counter, as it is designed to sit above the heel.
%DIFDELCMD < An additional heel counter was secured to the inside of the pressure bladder beneath the rearmost BOA dial.
%DIFDELCMD < The thickness of the polymer heel counter was determined through iterative testing; polymer thickness was increased until it interfered with donning and doffing.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The heel counter is used in footwear to provide a stiffer rear foot support \mbox{%DIFAUXCMD
...
\DIFaddbegin \DIFadd{An additional heel counter was designed based on the dynamic foot morphology model (Chapter \ref{sec:SA2}).
...
\DIFaddend \hypertarget{sole-1}{%
\subsection{Sole}\label{sole-1}}

\DIFdelbegin %DIFDELCMD < \DIFadd{The sole of the boot provides support, traction, and protection to the wearer.
%DIFDELCMD < As the prototype spacesuit boot will only be constructed for laboratory testing, a simple sole was constructed to represent the height that a manufactured sole would have.
%DIFDELCMD < A sole was designed by our collaborators }\DIFaddend %%%
\DIFdelend \DIFaddbegin \DIFadd{The sole of the boot provides support, traction, and protection to the wearer.
...
A sole was designed by our collaborators }\DIFaddend at the University of Oregon \DIFdelbegin %DIFDELCMD < \DIFdelbegin \DIFdel{Portland to ensure the current art of footwear design in incorporated into the fabrication of the boot.
%DIFDELCMD < This ensures that the framework is implemented successfully in its translation into a spacesuit boot prototype.
%DIFDELCMD < An additional collaboration between CU and BOA Technologies may incorporate the use of BOA laces into the upper restraint layer to capture the midfoot.
%DIFDELCMD < }\DIFdelend \DIFaddbegin \DIFadd{Sports Product Design Program to fit the last used for the prototype spacesuit boot design.
%DIFDELCMD < A 3D model of the sole is shown in fig.~\ref{fig:SA3-solemold}.
%DIFDELCMD < }\DIFaddend 
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Sports Product Design Program to fit the last used for the novel spacesuit boot design.
...
\DIFaddbegin \hypertarget{summary-4}{%
\subsection{Summary}\label{summary-4}}

\DIFadd{The novel spacesuit boot's design summarized in figure \ref{fig:SA3-lacingoverview}.
...
\DIFaddend \hypertarget{novel-planetary-spacesuit-boot-construction}{%
\section{Novel Planetary Spacesuit Boot Construction}\label{novel-planetary-spacesuit-boot-construction}}

\DIFdelbegin %DIFDELCMD < \DIFadd{The design of the novel planetary spacesuit boot was constructed into a prototype spacesuit boot with a pressure bladder and sole (fig.~\ref{fig:SA3-proto_boot}).
%DIFDELCMD < A heat-sealable urethane coated nylon (Seattle Fabrics, Seattle WA) was used as the material for the pressure bladder \mbox{%DIFAUXCMD
%DIFDELCMD < \citep{Harris2001}}\hspace{0pt}%DIFAUXCMD
%DIFDELCMD < .
%DIFDELCMD < Discussions with spacesuit engineers suggested 400 Denier nylon for its strength.}\\
%DIFDELCMD < \DIFadd{The material can be made air-tight by heat-sealing with a mini-iron (Clover Needlecraft, Los Angeles CA) which can reach a maximum of 580 degrees Fahrenheit.
%DIFDELCMD < Trial and error found that applying the iron at a temperature as low as 390 degrees Fahrenheit resulted in a strong bond after 20 seconds of heat application.
%DIFDELCMD < As only one side of the material is coated in urethane, and the urethane can only be bonded to another layer of urethane, care must be taken in the patterning to ensure that each panel is sealable.
%DIFDELCMD < It was decided that the urethane-coated side of the patterns will be facing towards the foot, as it is done in current spacesuit construction.
%DIFDELCMD < Panels were sealed together with another strip of pressure bladder material (fig.~\ref{fig:SA3-seam}).
%DIFDELCMD < This technique was applied to each section of the pressure bladder, with specific details regarding construction below.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Much effort was required to translate the novel planetary spacesuit boot design into a functional prototype, as shown in figure \ref{fig:SA3-proto_boot}.
The following sections outline the manufacturing process for constructing the boot prototype.
}\DIFaddend 

\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \hypertarget{fig:SA3-seam}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics{../fig/SA3/BladderSeam.png}
%DIFDELCMD < \caption{Two pressure bladder patterns (left, right) are connected by a single strip of material (middle) at the team. This strip is heat-sealed to both patterns, creating an air-tight seam.}\label{fig:SA3-seam}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \hypertarget{fig:SA3-proto_boot}{%
\centering
...
\DIFaddbegin \DIFadd{A heat-sealable urethane coated nylon (Seattle Fabrics, Seattle WA) was used as the material for the pressure bladder \mbox{%DIFAUXCMD
\citep{Harris2001}}\hspace{0pt}%DIFAUXCMD
.
...
\DIFadd{Numerous coupons were made to test how long heat was required to achieve a strong seal between the nylon fabric.
...
\DIFadd{As only one side of the material is coated in urethane, and the urethane can only be bonded to another layer of urethane, care must be taken in the patterning to ensure that each panel is sealable.
...
\DIFaddend \begin{figure}
\DIFdelbeginFL %DIFDELCMD < \hypertarget{fig:SA3-proto_boot}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.7\textwidth,height=\textheight]{../fig/SA3/Proto_Spaceboot.jpg}
%DIFDELCMD < \caption{Prototype planetary spacesuit boot constructed with BOA lacing, ankle rolling convolutes, and a heel counter (not shown).}\label{fig:SA3-proto_boot}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \hypertarget{fig:SA3-seam}{%
\centering
...
\DIFdelbegin %DIFDELCMD < \DIFadd{Ankle convolute construction starts with a flat panel for each convolute segment, as summarized in fig.~\ref{fig:SA3-convoluteflat}.
%DIFDELCMD < The convolute assembly was constructed by folding over the top of the convolute segments for the two convolute segments with a 50mm overlap.
%DIFDELCMD < This results in a convolute segment height of 100mm.
%DIFDELCMD < Then, the bottom of the preceding convolute segment is sewn to the folded-over top of the succeeding convolute segment using a straight stitch at 1.0 mm pitch, at both the overlap itself and 1 cm above the overlap.
%DIFDELCMD < The stitches are then heat-sealed.
%DIFDELCMD < This process is repeated for the next convolute segment.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Ankle convolute construction starts with a flat panel for each convolute segment, as summarized in figure \ref{fig:SA3-convoluteflat}.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{A restraint strap is constructed from a 3/4" in canvas fabric which is folded in half and stitched on one side.
%DIFDELCMD < The restraint strap's total height is 200mm.
%DIFDELCMD < Two restraint straps are used for each convolute assembly, and are placed at 25\% and 75\% of the flat panel convolute assembly's width.
%DIFDELCMD < As a result, these restraint straps will be on the medial and lateral side when the convolutes are fully assembled.
%DIFDELCMD < The restraint straps are stitched to the junctions between convolute segments by using a straight stitch at the overlap and 1cm above the overlap.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{A restraint strap is constructed from a 3/4" in canvas fabric which is folded in half and stitched on one side.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{Once all three convolute segments and the restraint straps are attached, the two ends of the segments are brought together to make the flat panel into a circular convolute assembly.
%DIFDELCMD < A strip of urethane-coated nylon is then used to seal the seam on the inside.
%DIFDELCMD < 3D printer filament is used for the bands of the rolling convolute.
%DIFDELCMD < The filament is inserted into the gap between the stitching at the convolute segment junctions.
%DIFDELCMD < The filament is trimmed to length at the seam.
%DIFDELCMD < A 1.5cm strip of canvas material is glued to the bottom of each convolute segment to ensure that the convolute's overlap actuates properly.
%DIFDELCMD < Without this canvas strip, the succeeding convolute segment was found to expand under pressurization and prevent the preceding convolute segment from rolling over it.
%DIFDELCMD < The ankle convolute assembly is now ready to be attached to the upper.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Once all three convolute segments and the restraint straps are attached, the two ends of the segments are brought together to make the flat panel into a circular convolute assembly.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{Patterns for the upper and toe box expected to interface with the pattern for the sole are extended by 1 cm to provide a seam to attach to the sole.
%DIFDELCMD < This avoids having to use a strip of urethane coated nylon on the interior of these seams, which is at a sharp angle and proved difficult to manufacture.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Patterns for the upper and toe box expected to interface with the pattern for the sole are extended by 1 cm to provide a seam to attach to the sole (fig.~\ref{fig:SA3-cutpatterns}).
...
\DIFdelbegin %DIFDELCMD < \DIFaddend %%%
\DIFdel{The }%DIFDELCMD < \DIFdelbegin \DIFdel{final design of the boot will aim to fit a range of individual foot measures.
%DIFDELCMD < As components of the boot are designed, the foot measure range they accommodate will be defined.
%DIFDELCMD < When all components are integrated into the final boot design, these foot measures will be used to recruit subjects who fit the boot; allowing for design evaluation in SA4}\DIFdelend \DIFaddbegin \DIFadd{patterns for the toe box are simply assembled and sealed together by using strips of urethane-coated nylon at the seams.
%DIFDELCMD < Construction of the upper needs to account for the instep lacing feature.
%DIFDELCMD < The inferior upper pattern is extended by the dimensions of the overlap needed for the instep lacing: 1cm near the MTP joint and 5 cm near the heel.
%DIFDELCMD < The top of the overlap is folded over and the superior upper pattern is stitched to the fold.
%DIFDELCMD < This is done for both the lateral and medial upper patterns.
%DIFDELCMD < The lateral and medial upper patterns are attached at the heel with a strip of urethane coated nylon.
%DIFDELCMD < Then, 3D printer filament is inserted into the gap between the stitches to provide structure to the convolute.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The patterns for the toe box are simply assembled and sealed together by using strips of urethane-coated nylon at the seams.
...
\DIFaddend \hypertarget{sole-construction}{%
\subsection{Sole Construction}\label{sole-construction}}

\DIFdelbegin %DIFDELCMD < \DIFadd{Ethylene-Vinyl Acetate foam is a common material used for sole construction in sneakers and daily shoes.
%DIFDELCMD < Collaborators at the University of Oregon Sports Product Design Program informed us that their prototyping process for most footwear involved using Smooth-On Flex Form-it 17 (Smooth-On, Macungie PA) expanding foam, which provides a similar bounce and deflection as Ethylene-Vinyl Acetate foam.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Ethylene-Vinyl Acetate foam is a common material used for sole construction in sneakers and daily shoes.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{A mold for the sole was designed to allow for casting from the expanding foam.
%DIFDELCMD < The sole was provided as a 3D mesh file (fig.~\ref{fig:SA3-solemold}).
%DIFDELCMD < A negative of the sole was created using the Blender software, where a rectangular box mesh which was bigger than the sole was placed over it and the difference between the two meshes was taken.
%DIFDELCMD < A seam was added to this box so that it can be separated into two halve.
%DIFDELCMD < Three holes were placed in the top half of the box above the centerline of the sole negative, allowing for the foam to be poured in before it cures.
%DIFDELCMD < In addition, four holes were placed at the corners of the box, allowing the box to be clamped together with bolts and thus sealing the mold.
%DIFDELCMD < This mold was then 3D-printed out of PLA material, and is shown in fig.~\ref{fig:SA3-solemoldhalves}.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{A mold for the sole was designed to allow for casting from the expanding foam.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{Ease Release 2831 (Smooth-On, Macungie PA) was applied to the 3D mold.
%DIFDELCMD < The expanding foam was mixed, poured into the mold, and allowed to fully cure as per the manufacturer's instructions.
%DIFDELCMD < The mold was then unclamped, and the sole was peeled out of the mold.
%DIFDELCMD < Any flash, excess material from the molding process, was then cut off with a knife.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Ease Release 2831 (Smooth-On, Macungie PA) was applied to the 3D mold.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{The pressure bladder is attached to the sole through a welting process.
%DIFDELCMD < The seam from the attachment of the pressure bladder's upper and sole patterns is sewn to the sole as shown in }{[}\DIFadd{Fig TBD}{]}\DIFadd{.
%DIFDELCMD < This process is reinforced with hot glue.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The pressure bladder is attached to the sole through a welting process.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{The BOA lacing guides are secured to the pressure bladder and sole through stitching.
%DIFDELCMD < The guides are first perforated with four holes, and then hand stitched to the pressure bladder and sole.
%DIFDELCMD < The BOA lacing dial is glued to the top of the upper most convolute segment.
%DIFDELCMD < The lace tubing is press-fit into the dial and the first set of guides.
%DIFDELCMD < The lace tubing is also glued to the canvas straps at the bottom of upper-most convolute segment to ensure it stays secure.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The BOA lacing guides are secured to the pressure bladder and sole through stitching.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{The heel counter made by casting a thin (3 mm) layer of Elite Double 16 dental polymer (Zhermack SpA, Badia Polesine (RO), Italy).
%DIFDELCMD < This layer was trimmed to size to fit on the interior of the boot beneath the lacing convolute.
%DIFDELCMD < The heel counter was secured to the interior of the boot with hot glue and tape.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The heel counter made by casting a thin (3 mm) layer of Elite Double 16 dental polymer (Zhermack SpA, Badia Polesine (RO), Italy).
...
\DIFdelbegin %DIFDELCMD < \DIFadd{While the urethane-coated nylon has an air-tight seam when heat-sealed properly, manufacturing tolerances and wear from use of the boot were found to create leak paths in the prototype spacesuit boot's pressure bladder.
%DIFDELCMD < It was found that hot glue was an adequate sealant for the pressures used in testing.
%DIFDELCMD < Therefore, a thin layer of hot glue was applied to all seams in the pressure bladder.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{While the urethane-coated nylon has an air-tight seam when heat-sealed properly, manufacturing tolerances and wear from use of the boot were found to create leak paths in the prototype spacesuit boot's pressure bladder.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{A ``classic'' spacesuit boot representing the current planetary spacesuit boot prototypes was also constructed (fig.~\ref{fig:SA3-classic_boot}).
%DIFDELCMD < Similar construction techniques were used as the prototype planetary spacesuit boot: the same sole was used for both boots, and the upper for both boots was designed around the same last.
%DIFDELCMD < The convolutes for the classic spacesuit boot were constructed with the same height, overlap, and restraint strap, but the convolutes were left flat to the foot's plantar surface, as they are in current planetary spacesuit boot prototypes \mbox{%DIFAUXCMD
%DIFDELCMD < \citep{Ross2002}}\hspace{0pt}%DIFAUXCMD
%DIFDELCMD < .
%DIFDELCMD < The upper for the classic spacesuit boot does not have any lacing; the superior and anterior upper patterns were sealed at the seam separating them.
%DIFDELCMD < Some current planetary spacesuit boot prototypes, such as the one provided by David Clark, do have lacing features, but it is not clear how this lacing
%DIFDELCMD < This ensure that the two boots are identical except for the three novel features in the prototype boot: angled ankle convolutes, lacing, and a heel counter}\DIFaddend %%%
\DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{To evaluate the design of the proposed novel spacesuit boot, a comparison ``classic'' spacesuit boot was fabricated, based on current designs to the extent possible (fig.~\ref{fig:SA3-classic_boot}).
...
The convolutes for the classic spacesuit boot were constructed with the same height, and overlap, but the convolutes were left flat to the foot's plantar surface, as they are in current planetary spacesuit boot prototypes \mbox{%DIFAUXCMD
\citep{Ross2002}}\hspace{0pt}%DIFAUXCMD
.
...
\DIFdelbegin %DIFDELCMD < \DIFaddbegin %%%
\DIFdelend \begin{figure}
\DIFdelbeginFL %DIFDELCMD < \hypertarget{fig:SA3-classic_boot}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.7\textwidth,height=\textheight]{../fig/SA3/Classic_Spaceboot.jpg}
%DIFDELCMD < \caption{Classic spacesuit boot which was constructed with similar techniques as the prototype spacesuit boot, but without lacing, or a heel-counter.}\label{fig:SA3-classic_boot}
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \DIFaddend \hypertarget{summary-4}{%
%DIFDELCMD < \section{Summary}\label{summary-4}}
%DIFDELCMD < %%%
...
The only previously bounding requirement, the 2019 NASA SBIR solicitation for a new surface spacesuit boot, had only one requirement for ankle flexion/extension\DIFdelbegin \DIFdel{, which was validated in this paper.
...
}\DIFaddend This work provides a series of requirements based from previous biomechanics studies on foot motion while walking and hiking to \DIFdelbegin \DIFdel{provide proper fit and mobility through the spacesuit boot design.
}%DIFDELCMD < \DIFdelbegin \DIFdel{Prototypes constructed from this work will be validated in Specific Aim 4.
%DIFDELCMD < }\DIFdelend 
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{expand upon the provided NASA requirements.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \DIFdelbegin \DIFdel{As of Fall 2020, the literature review and biomechanical design framework have been completed.
%DIFDELCMD < Work is currently starting on the prototype spacesuit boot design and construction.
%DIFDELCMD < The literature review and biomechanical design framework have been presented in a paper published at the 2020 International Conference on Environmental Systems Meeting.
%DIFDELCMD < The final designof the }\DIFdelend \DIFaddbegin \DIFadd{These requirements, along with findings from the dynamic foot shape model in Chapter 5, were translated into a prototype spacesuit boot design.
%DIFDELCMD < This design features a novel lacing feature which accommodates the static and dynamic variability in instep height.
%DIFDELCMD < In addition, the prototype }\DIFaddend %%%
\DIFdel{boot }%DIFDELCMD < \DIFdelbegin \DIFdel{will be presented in a future journal paper and conference presentations.
...
%DIFDELCMD < The prototype boot modifies the ankle convolutes found in current planetary spacesuit boots by tilting their plane towards the heel, thus lowering the joint center to be better indexed with the operator's ankle joint.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{These requirements, along with findings from the dynamic foot shape model in Chapter \ref{sec:SA2}, were translated into a prototype spacesuit boot design.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{The prototype boot was constructed using an urethane coated nylon for the pressure bladder.
%DIFDELCMD < A sole constructed from expanding foam was integrated into the design.
%DIFDELCMD < Lacing for the prototype boot was provided by BOA.
%DIFDELCMD < }\DIFaddend 
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The prototype boot was constructed using an urethane coated nylon for the pressure bladder.
A sole constructed from expanding foam was integrated into the design.
Lacing for the prototype boot was provided by BOA.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \DIFaddbegin \DIFadd{A classic planetary spacesuit boot was also constructed, but omitted the novel features of the prototype spacesuit boot.
%DIFDELCMD < No lacing or heel counter was included in the classic spacesuit boot.
%DIFDELCMD < Ankle convolutes were left parallel to the plantar surface.
%DIFDELCMD < However, this boot was constructed using the same last, resulting in similarly shaped patterns.
%DIFDELCMD < In addition, the sole used for this boot is identical.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{A classic planetary spacesuit boot was also constructed, but omitted the novel features of the prototype spacesuit boot.
...
\DIFdelbegin %DIFDELCMD < \DIFadd{This ensures that the novel features of the prototype spacesuit boot can be compared to the classic spacesuit boot through evaluation.
%DIFDELCMD < This evaluation will be carried out in Chapter 7.
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \DIFaddend \hypertarget{specific-aim-4-evaluate-novel-planetary-spacesuit-boot-design-for-fit-comfort-and-mobility}{%
%DIFDELCMD < \chapter{Specific Aim 4: Evaluate novel planetary spacesuit boot design for fit, comfort, and mobility}\label{specific-aim-4-evaluate-novel-planetary-spacesuit-boot-design-for-fit-comfort-and-mobility}}
%DIFDELCMD < 

%DIFDELCMD < \hypertarget{introduction-3}{%
%DIFDELCMD < \section{Introduction}\label{introduction-3}}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The pressurized }\DIFdelend \DIFaddbegin \DIFadd{The novel pressurized spacesuit }\DIFaddend boot prototype developed in Specific Aim 3 \DIFdelbegin \DIFdel{will need to be validated to test the }\DIFdelend \DIFaddbegin \DIFadd{was validated for fit and comfort through subject evaluation.
Results of the validation can help answer the }\DIFaddend main hypothesis of this thesis: a spacesuit boot designed with dynamic foot morphology will provide increased compatibility between the spacesuit and operator.
\DIFdelbegin \DIFdel{The gold-standard of performance for EVA mobility would be an unpressurized hiking boot.
Current MK III boots feature only a convoluted ankle joint \mbox{%DIFAUXCMD
\citep{Ross2002}}\hspace{0pt}%DIFAUXCMD
, which can be constructed as a pressurized hiking boot with a convoluted ankle joint.
...
Regardless of test-setup, }\DIFdelend \DIFaddbegin \DIFadd{In a terrestrial environment not requiring pressurized spacesuits, EVA tasks are best performed with a work boot.
Therefore, an unpressurized laced work boot was used as the gold standard to compare the novel spacesuit boot to.
As the novel spacesuit boot features lacing, }\DIFaddend the \DIFdelbegin \DIFdel{main objectives of this work are:
}\DIFdelend \DIFaddbegin \DIFadd{unpressurized work boot was compared in both laced and unlaced conditions.
...
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{Construct interface between test boots and test environments
}%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{Evaluate the mobility of the novel design against a standard hiking boot and pressurized hiking boot}%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{Evaluate the comfort of the novel design against a standard hiking boot and pressurized hiking boot }%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{Evaluate the novel design for its ability to reduce instances of heel lift against a standard hiking bootand pressurized hiking boot }
...
\DIFdelbegin %DIFDELCMD < \hypertarget{test-interface-construction}{%
%DIFDELCMD < \section{Test Interface Construction}\label{test-interface-construction}}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Interfaces will need to be constructed between the boots to be tested and the test environments, as shown in fig.
~\ref{fig:SA4-interface} .
For the glovebox testing, the interface will need to allow for pressurization of the prototype bootand the MK III boot}\DIFdelend \DIFaddbegin \DIFadd{A pressurization interface was constructed to allow a pressurized boot to be worn by a subject during testing.
...
This allows for the performance of the boots to be isolated to the boots themselves, and not to the presence or absence of a spacesuit}\DIFaddend .
\DIFdelbegin \DIFdel{When a vacuum is pulled inside the glovebox, the bootwill become pressurized with the ambient air .
The standard hiking bootwill also need an interface to the glovebox to offer similar testing conditions, but will not need to be pressurized.
}\DIFdelend 

\DIFdelbegin \DIFdel{Another potential interface will be mate the pressurized boots to }\DIFdelend \DIFaddbegin \hypertarget{interface-layers}{%
\subsection{Interface layers}\label{interface-layers}}

\DIFadd{A base layer of neoprene is heat-sealed to }\DIFaddend the \DIFdelbegin \DIFdel{shank of the wearer.
This interface will require some sort of seal above the wearer' s ankle .
Due to expected imperfections in the seal, }\DIFdelend \DIFaddbegin \DIFadd{upper-most ankle convolute.
This neoprene layer is patterned as a staircase (fig.~\ref{fig:SA4-baseseal}), with the dorsal side having a height of 3 cm, and }\DIFaddend the \DIFdelbegin \DIFdel{boot will most likely be fed with constant pressurized air to maintain a constant pressure.
...
}\DIFdelend \DIFaddbegin \DIFadd{ventral side having a height of 6 cm.
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{A neoprene and canvas strap was constructed with velcro backings to assist in creating a seal around the neoprene base layer.
...
\DIFdelbegin \DIFdel{The hypotheses tested in this aim are as follows}\DIFdelend \DIFaddbegin \begin{figure}
...
\DIFadd{Two air lines were integrated into the boots just below the neoprene layer; a supply air line to provide pressurized air and a monitoring air line log the boot's interal pressure with a pressure gauge.
...
\DIFadd{The boot's monitoring line was connected to a differential pressure manometer (Amprobe MAN30, Amprobe, Everett WA).
...
\DIFadd{The pressurization interface was used on all subjects during boot evaluation.
...
\DIFadd{The EMU operates at 4.3 psi, and future planetary exploration suit concepts have called for suits to be operated at a higher 8.3 psi to reduce the pre-breathe time required for astronauts prior to getting into the suit \mbox{%DIFAUXCMD
\citep{Kosmo1988}}\hspace{0pt}%DIFAUXCMD
.
The ACES, which was not designed for EVA operations, operated at 3.5 psi \mbox{%DIFAUXCMD
\citep{Thomas2012}}\hspace{0pt}%DIFAUXCMD
.
Higher operational pressures may require increased effort to move suit components.
}

\DIFadd{It was originally desired to reach a pressure of 4.3 psi to replicate the spacesuit pressure in current EVA activities aboard the EMU.
...
\DIFadd{The goal of this evaluation was to assess fit and comfort of the novel spacesuit boot, as it compared to the classic spacesuit boot, unlaced work boot, and laced work boot.
...
Therefore, this experiment aimed to answer the following hypotheses}\DIFaddend :

\begin{itemize}
\tightlist
\item
  The novel \DIFdelbegin \DIFdel{boot design will provide equivalent mobility }\DIFdelend \DIFaddbegin \DIFadd{spacesuit boot design provides equivalent comfort }\DIFaddend compared to the \DIFdelbegin \DIFdel{standard hiking }\DIFdelend \DIFaddbegin \DIFadd{laced work }\DIFaddend boot and improved \DIFdelbegin \DIFdel{mobility }\DIFdelend \DIFaddbegin \DIFadd{comfort }\DIFaddend compared to the \DIFdelbegin \DIFdel{pressurized hiking }\DIFdelend \DIFaddbegin \DIFadd{unlaced work boot and classic pressurized spacesuit }\DIFaddend boot
\item
  The novel \DIFdelbegin \DIFdel{boot design will provide equivalent comfort }\DIFdelend \DIFaddbegin \DIFadd{spacesuit boot design provides equivalent heel performance }\DIFaddend compared to the \DIFdelbegin \DIFdel{standard hiking }\DIFdelend \DIFaddbegin \DIFadd{laced work }\DIFaddend boot and improved comfort compared to the \DIFdelbegin \DIFdel{pressurized hiking boot
}%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{The novel boot design will induce the same number of instances of heel lift as the hiking boot and fewer instances of heel lift compared to the pressurized hiking boot }\DIFdelend \DIFaddbegin \DIFadd{unlaced work boot and classic pressurized spacesuit boot
}\DIFaddend \end{itemize}

\DIFdelbegin \DIFdel{The independent variables used to test these hypothesis will be the different boots tested: the standard hiking boot, the pressurized hiking boot (representing current planetary spacesuit boot technology), and the novel spacesuit boot developed in SA3.
...
The Corlett-Bishop Discomfort Scale \mbox{%DIFAUXCMD
\citep{Corlett1976}}\hspace{0pt}%DIFAUXCMD
, Rating of Perceived Exertion Survey \mbox{%DIFAUXCMD
\citep{Borg1982}}\hspace{0pt}%DIFAUXCMD
, and Gravity compensation and performance scale (GCPS) \mbox{%DIFAUXCMD
\citep{Gernhardt2009, Norcross2009, Norcross2010} }\hspace{0pt}%DIFAUXCMD
will be used to assess subjective comfort of the boots.
Findings from Specific Aim }\DIFdelend \DIFaddbegin \hypertarget{methods-3}{%
\section{Methods}\label{methods-3}}

\hypertarget{subjects-1}{%
\subsection{Subjects}\label{subjects-1}}

\DIFadd{Five subjects with a self-reported shoe size between 8-10 US Male (9.5-11.5 US Female) were recruited to participate in this evaluation.
...
\caption{\label{tbl:foot_meas}\DIFadd{Summary of subjects' foot measurements}}\tabularnewline
\toprule
\DIFadd{Subject }& \DIFadd{Foot Length (cm) }& \DIFadd{Foot Width }& \DIFadd{Arch Length (cm) }& \DIFadd{Sex }\\
\midrule
\endfirsthead
\toprule
\DIFadd{Subject }& \DIFadd{Foot Length (cm) }& \DIFadd{Foot Width }& \DIFadd{Arch Length (cm) }& \DIFadd{Sex }\\
\midrule
\endhead
\DIFadd{FQ }& \DIFadd{25.5 }& \DIFadd{C }& \DIFadd{26 }& \DIFadd{F }\\
\DIFadd{FT }& \DIFadd{25.5 }& \DIFadd{D }& \DIFadd{25 }& \DIFadd{F }\\
\DIFadd{RM }& \DIFadd{25.5 }& \DIFadd{C-D }& \DIFadd{27 }& \DIFadd{M }\\
\DIFadd{TF }& \DIFadd{25.5 }& \DIFadd{D }& \DIFadd{25 }& \DIFadd{M }\\
\DIFadd{TQ }& \DIFadd{26.5 }& \DIFadd{D }& \DIFadd{27 }& \DIFadd{M }\\
\DIFadd{Average }& \DIFadd{25.7 \(\pm\) 0.4 }& & \DIFadd{26 \(\pm\) }\DIFaddend 1 \DIFaddbegin & \\
\bottomrule
\end{longtable}
...
\DIFadd{For each boot condition, subjects donned the test boot on their right foot.
...
\DIFadd{Subjects started by performing one set of five heel-raises, and were instructed to best synchronize their heel-raises between both feet.
...
\DIFadd{Each of the test boots had two markers placed on their medial side.
...
\DIFadd{For the heel-raises, subjects were instructed to stand centered between two Intel RealSense D415 cameras.
...
\DIFadd{Findings from Chapter \ref{sec:SA1} }\DIFaddend showed that IMUs may not be appropriate for detecting instances of heel-lift \DIFaddbegin \DIFadd{and characterizing heel performance}\DIFaddend .
Therefore, a force \DIFdelbegin \DIFdel{sensor will be placed in the heel of the boot, in as minimally intrusive position as possible, to detect instances of heel-lift.If possible, a sensor which can measure distance between the sole and heel will be selected for this application.
Instances of heel-lift will also be correlated to subjective comments, and future work may use the selected sensor to validate IMU usage in detecting heel-lift in the spacesuit}\DIFdelend \DIFaddbegin \DIFadd{sensitive resistor (FSR 402, Interlink Electronics, Camarillo CA) was embedded in the sole of the test boots fig.~\ref{fig:SA4-FSRembed}.
...
To align the sign of the FSR voltage to the sign of the heel rise magnitude, the voltage reading was inverted such that applying force to the FSR decreased the voltage}\DIFaddend .

\DIFdelbegin \DIFdel{Subjects will be recruited for this study whose feet fit the defined foot measure range of the }\DIFdelend \DIFaddbegin \begin{figure}
...
\DIFadd{Post-hoc calibration of the FSRs was conducted to characterize the voltage response to force.
...
Due to sensor breakage, the sensor in the }\DIFaddend novel spacesuit boot \DIFdelbegin \DIFdel{developed in SA3.
Subjects will be prescreened for their foot measures }\DIFdelend \DIFaddbegin \DIFadd{for subjects FT and TF was not calibrated, as it was replaced }\DIFaddend prior to being \DIFdelbegin \DIFdel{enrolled in the study.
A minimum of 5 subjectswill be enrolled in this study.
The order in which a subject tests each bootwill be counterbalanced to avoid carryover and fatigue effects.
}\DIFdelend \DIFaddbegin \DIFadd{used by the other subjects.
...
\DIFdelbegin \DIFdel{All three boots will be mated to the glovebox, and a vacuum will be pulled for both pressurized bootsto allow ambient air to pressurize them.
...
Motions will be performed both unloaded, where the boot is free in }\DIFdelend \DIFaddbegin \begin{figure}
...
\DIFadd{A Python script was written to sync the FSR and video data, collecting frames from both cameras and the FSR voltage at the same time.
...
Computational power limited the sample rate of }\DIFaddend the \DIFdelbegin \DIFdel{air, and loaded}\DIFdelend \DIFaddbegin \DIFadd{FSR and video data to 14 Hz.
While this is much lower than typical biomechanics sample rates, it is still within the frequency content of gait \mbox{%DIFAUXCMD
\citep{Antonsson1985}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFadd{While video and FSR data was collected during the short walk, this data was not analyzed due to the video's limited field-of-view, thus only capturing one step in each walk.
}

\hypertarget{survey-measures}{%
\subsection{Survey Measures}\label{survey-measures}}

\DIFadd{After completion of the exercises in each boot, subjects filled out a survey assessing heel performance, discomfort, and exertion.
...
Low heel performance was described as analogous to a flip-flop}\DIFaddend , where the \DIFdelbegin \DIFdel{boot is pushing against a flat floor.
This is consistent with previous planetary boot testing methodology \mbox{%DIFAUXCMD
\citep{Ross2002}}\hspace{0pt}%DIFAUXCMD
.
The subject will also perform a series of heel lifts to test for }\DIFdelend \DIFaddbegin \DIFadd{shoe does not lift when the heel is lifted and there is high heel-lift.
High heel performance was described as analogous to a well fitted sneaker, where the shoe instantly responds to the subjects heel being lifted and there is minimal to no }\DIFaddend heel-lift.
\DIFaddbegin \DIFadd{The Corlett-Bishop Discomfort Scale \mbox{%DIFAUXCMD
\citep{Corlett1976} }\hspace{0pt}%DIFAUXCMD
was used to assess subject discomfort; the survey was modified and limited to 10 areas below the knees of the subject.
...
The Rating of Perceived Exertion (RPE) \mbox{%DIFAUXCMD
\citep{Borg1982} }\hspace{0pt}%DIFAUXCMD
was used to assess how much effort was required to perform the motions.
This question asks subjects to report the amount of effort required to perform the heel-rise and walking tasks.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \hypertarget{stretch-testing-goals}{%
%DIFDELCMD < \section{Stretch Testing Goals}\label{stretch-testing-goals}}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The glovebox testing will provide initial insight into comfort and mobility, but will be limited in translation as it does not include walking.
If a pressurized seal around the ankle is achievable, a biomechanical walking evaluation of }\DIFdelend \DIFaddbegin \DIFadd{After performing the exercises in all four boots, subjects were then asked to rank the boots from best to worst with respect to overall performance and comfort.
}
...
\DIFadd{Heel position, as measured by the Kinovea software, was used to identify heel-off initiation time and maximum heel-rise magnitude for both the left (control) and right (test) shoe for each boot condition's heel-rise set.
For all sets, the first heel-rise was not analyzed.
Python's }\texttt{\DIFadd{scipy.signal.find\_peaks}} \DIFadd{peak detection algorithm was used to identify the peak heel positions for each heel-rise with a width of 5 samples.
...
\DIFadd{Heel-off initiation time was identified by first inverting the heel-position signal and then identifying the signal peaks with the identical parameters for each side as for the maximum heel-rise magnitude analysis.
These peaks are the foot-flat phases of }\DIFaddend the \DIFdelbegin \DIFdel{three boots can occur.
...
The boot will be pressurized through a portable compressor, with a pressure line running to the subject and routed to the boot}\DIFdelend \DIFaddbegin \DIFadd{heel-rise motion for each heel-rise.
...
The point at which the velocity signal crossed 1 cm/s was identified as the heel-off initiation time for that heel-rise}\DIFaddend .
\DIFdelbegin \DIFdel{The stated hypotheses and independent variables will remain the same for walking testing, but joint torques will be added as a dependent variable.
}\DIFdelend 

\DIFdelbegin \DIFdel{Walking will occur on a flat walkway, where the subject will walk across force plates to measure ground reaction forces.
Optical motion capture will be used to capture segment kinematics of }\DIFdelend \DIFaddbegin \DIFadd{The FSR voltage was recorded at each heel-off initiation time.
...
Then, Python's }\texttt{\DIFadd{scipy.signal.find\_peaks}} \DIFadd{algorithm was used to identify peaks with a prominence of 0.5 V and a width of 5 samples.
As the FSR signal was quite noisy at its peak, a window of 50\% of the peak's width from }\DIFaddend the \DIFdelbegin \DIFdel{lower torso, allowing for calculation of joint range-of-motions for mobility assessment.
...
The aforementioned subjective surveys will be administered following data collection with each boot}\DIFdelend \DIFaddbegin \DIFadd{peak's highest value was isolated.
The average FSR voltage in this window was then noted as the FSR peak voltage for that heel-rise}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \hypertarget{summary-5}{%
%DIFDELCMD < \section{Summary}\label{summary-5}}
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \DIFdelbegin \DIFdel{To date, the following contributions have been completed:
%DIFDELCMD < }\DIFdelend \DIFaddbegin %%%
...
\begin{itemize}%DIFAUXCMD
%DIF < DIFAUXCMD
%DIF < DIFDELCMD < \tightlist
%DIF < DIFDELCMD < \item
%DIFDELCMD < \item%%%
\item%DIFAUXCMD
%DIF < DIFAUXCMD
%DIF < DIFDELCMD <   %%%
%DIFDELCMD < \DIFdel{Showed drawbacks and identified areas of future implementation for how IMUs could be used to detect instances of heel-lift
%DIFDELCMD < }%%%
%DIF < DIFDELCMD < \item
%DIFDELCMD < \item%%%
\item%DIFAUXCMD
%DIF < DIFAUXCMD
%DIF < DIFDELCMD <   %%%
%DIFDELCMD < \DIFdel{A novel software library to use multiple commercial depth cameras as a cost-effective 4D scanner, capable of collecting body shape changes at 90 frames-per-second
%DIFDELCMD < }%%%
%DIF < DIFDELCMD < \item
%DIFDELCMD < \item%%%
\item%DIFAUXCMD
%DIF < DIFAUXCMD
%DIF < DIFDELCMD <   %%%
%DIFDELCMD < \DIFdel{A parametric statistical shape model that predicts dynamic foot shape during stance phase as a function of anthropometry and kinematics
%DIFDELCMD < }%%%
%DIF < DIFDELCMD < \item
%DIFDELCMD < \item%%%
\item%DIFAUXCMD
%DIF < DIFAUXCMD
%DIF < DIFDELCMD <   %%%
%DIFDELCMD < \DIFdel{A design framework integrating dynamic foot shape knowledge with known foot mobility to design a novel spacesuit boot to improve fit and comfort
%DIFDELCMD < }
...
%DIFDELCMD < %%%
%DIF < DIFDELCMD < %%%
%DIFDELCMD < \DIFdel{This thesis aims to provide the following contributions during the remainder of its course:
%DIFDELCMD < }%%%
%DIF < DIFDELCMD < 
%DIFDELCMD < 
...
%DIF < DIFAUXCMD
%DIF < DIFDELCMD <   %%%
%DIFDELCMD < \DIFdel{Quantification of dynamic changes in instep height and instep girth as they relate to their distribution in the global population
%DIFDELCMD < }%%%
%DIF < DIFDELCMD < \item
%DIFDELCMD < \item%%%
\item%DIFAUXCMD
%DIF < DIFAUXCMD
%DIF < DIFDELCMD <   %%%
%DIFDELCMD < \DIFdel{A novel spacesuit boot prototype developed from the framework that aims for better fit and comfort, and specifically a reduction in heel-lift
%DIFDELCMD < }%%%
%DIF < DIFDELCMD < \item
%DIFDELCMD < \item%%%
\item%DIFAUXCMD
%DIF < DIFAUXCMD
%DIF < DIFDELCMD <   %%%
%DIFDELCMD < \DIFdel{Evaluation of the novel spacesuit boot for comfort and mobility against current planetary spacesuit boot technology and a gold-standard hiking boot
%DIFDELCMD < }
...
%DIFDELCMD < %%%
%DIF < DIFDELCMD < %%%
%DIFDELCMD < \DIFdel{Through these contributions, this thesis aims to show the efficacy of dynamic body-shape models in improving spacesuit component design for fit, comfort, and mobility, thereby answering its hypothesis.
%DIFDELCMD < While the efforts of this thesis were demonstrated on a planetary spacesuit boot, the presented work aims to serve as a foundation for other spacesuit component designs where fit may be an issue, such as the HUT and gloves.
%DIFDELCMD < The work presented in this thesis also aims to be translational for terrestrial footwear, contributing a new capture tool and modeling technique to improve footwear fit and comfort for a variety of activities.
%DIFDELCMD < }%%%
%DIF < DIFDELCMD < 
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < DIFDELCMD < \hypertarget{publication-plan}{%
%DIF < DIFDELCMD < \section{Publication Plan}\label{publication-plan}}
%DIF < DIFDELCMD < 
%DIFDELCMD < 
...
%DIF < DIFDELCMD < %%%
%DIFDELCMD < \DIFdelend %%%
\DIFdelend Table \ref{tbl:pubs} outlines the peer-reviewed conference and journal papers \DIFdelbegin %DIFDELCMD < \DIFaddbegin \DIFadd{resulting }\DIFaddend %%%
\DIFdelend \DIFaddbegin \DIFadd{resulting }\DIFaddend from this thesis work.
