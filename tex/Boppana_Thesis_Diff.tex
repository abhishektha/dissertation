...
%DIF DEL Boppana_Dissertation_Draft01.10.00.tex   Mon Feb 28 07:55:06 2022
%DIF ADD Boppana_Thesis2.tex                      Mon Mar  7 07:58:46 2022
...
\DIFdelbegin %DIFDELCMD < \hypertarget{specific-aim-3-define-a-design-process-integrating-dynamic-foot-morphology-data-for-a-novel-spacesuit-boot}{%
%DIFDELCMD < \chapter{Specific Aim 3: Define a design process integrating dynamic foot morphology data for a novel spacesuit boot}\label{specific-aim-3-define-a-design-process-integrating-dynamic-foot-morphology-data-for-a-novel-spacesuit-boot}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{sec:SA3}{%
...
%DIFDELCMD < \hypertarget{sec:SA4}{%
%DIFDELCMD < \chapter{Specific Aim 4: Evaluate novel planetary spacesuit boot design for fit and comfort}\label{sec:SA4}}
%DIFDELCMD < 
...
%DIFDELCMD < \hypertarget{pressurization-test-interface}{%
%DIFDELCMD < \section{Pressurization Test Interface}\label{pressurization-test-interface}}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend A pressurization interface was constructed to allow a pressurized boot to be worn by a subject during testing.
This interface was common between the classic and novel pressurized spacesuit boots.
\DIFdelbegin \DIFdel{The interface was designed to allow the }\DIFdelend \DIFaddbegin \DIFadd{This interface is not designed to integrate with a spacesuit, but rather for a }\DIFaddend user to wear a pressurized spacesuit boot without having to wear an entire spacesuit, much like wearing \DIFdelbegin \DIFdel{the control work boot}\DIFdelend \DIFaddbegin \DIFadd{a terrestrial shoe}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \hypertarget{interface-layers}{%
%DIFDELCMD < \subsection{Interface layers}\label{interface-layers}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{interface-layers}{%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{air-feed-and-monitoring}{%
%DIFDELCMD < \subsection{Air Feed and Monitoring}\label{air-feed-and-monitoring}}
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{interface-performance}{%
%DIFDELCMD < \subsection{Interface Performance}\label{interface-performance}}
%DIFDELCMD < %%%
...
\DIFaddbegin \hypertarget{summary-5}{%
\section{Summary}\label{summary-5}}

\DIFadd{This analysis outlined a framework for designing a new spacesuit boot with an emphasis on fit and mobility during gait.
The framework aims to reduce the risk of spacesuit boot injury by developing a process to design a spacesuit boot.
It is expected that focusing a design on fit and mobility will reduce the occurrence of heel-lift and contact injuries.
}

\DIFadd{This framework therefore serves as bounding requirements to ensure future spacesuit footwear does not inhibit natural foot motion or cause discomfort due to incompatibilities between foot and shoe shape.
The only previously bounding requirement, the 2019 NASA SBIR solicitation for a new surface spacesuit boot, had only one requirement for ankle flexion/extension.
This work provides a series of requirements based from previous biomechanics studies on foot motion while walking and hiking to expand upon the provided NASA requirements.
}

\DIFadd{These requirements, along with findings from the dynamic foot shape model in Chapter \ref{sec:SA2}, were translated into a prototype spacesuit boot design.
This design features a novel lacing feature which accommodates the static and dynamic variability in instep height.
In addition, the prototype boot also features a heel counter, which should assist in ensuring the heel does not slip out of the boot during walking.
The prototype boot modifies the ankle convolutes found in current planetary spacesuit boots by tilting their plane towards the heel, thus lowering the joint center to be better indexed with the operator's ankle joint.
}
...
\DIFadd{A classic planetary spacesuit boot was also constructed, but omitted the novel features of the prototype spacesuit boot.
No lacing or heel counter was included in the classic spacesuit boot.
Ankle convolutes were left parallel to the plantar surface.
However, this boot was constructed using the same last, resulting in similarly shaped patterns.
In addition, the sole used for this boot is identical.
...
\DIFadd{The novel pressurized spacesuit boot prototype developed in Specific Aim 3 was validated for fit and comfort through subject evaluation.
Results of the validation can help answer the main hypothesis of this thesis: a spacesuit boot designed with dynamic foot morphology will provide increased compatibility between the spacesuit and operator.
In a terrestrial environment not requiring pressurized spacesuits, EVA tasks are best performed with a work boot.
Therefore, an unpressurized laced work boot was used as the gold standard to compare the novel spacesuit boot to.
As the novel spacesuit boot features lacing, the unpressurized work boot was compared in both laced and unlaced conditions.
...
\DIFaddend \hypertarget{experimental-design}{%
...
The order of the test boots was counterbalanced \DIFdelbegin \DIFdel{across all subjects .
}\DIFdelend \DIFaddbegin \DIFadd{within the pressurized and unpressurized categories; subjects started their trials with the unpressurized boots and then proceeded to the pressurized boots.
...
}\DIFaddend Subjects were instructed to bring their own low-top sneaker to wear on their left \DIFdelbegin \DIFdel{shoe}\DIFdelend \DIFaddbegin \DIFadd{foot}\DIFaddend ; a low-top sneaker does not limit the amount of heel-rise when compared to a high-top shoe, and therefore serves as a control between all test boot conditions.
...
\DIFdelbegin \DIFdel{However, for }\DIFdelend \DIFaddbegin \DIFadd{In addition, there was little confidence that the calibration curve is completely representative of a foot on the sensor, as the foot may shift while maintaining contact and this could not be accurately characterized by a single sensor.
...
For }\DIFaddend all boots, there is a strong inflection point in the response at 0.5V, where voltages below 0.5V covered a wide range of applied force.
\DIFdelbegin \DIFdel{Therefore, it }\DIFdelend \DIFaddbegin \DIFadd{It }\DIFaddend was decided to characterize a voltage of 3.3V as ``no contact'', voltages between 0.5 and 3.3 V as ``mild contact'', and voltages below 0.5V as ``full contact''.

\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \hypertarget{fig:SA4-FSRcal}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.9\textwidth,height=\textheight]{../fig/SA4/FSR_cal.png}
%DIFDELCMD < \caption{Voltage response curves of the FSR to an applied force from a force gauge, for each of the three boots.}\label{fig:SA4-FSRcal}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \hypertarget{fig:SA4-FSRcal}{%
\centering
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \hypertarget{fig:SA4-CB}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.4\textwidth,height=\textheight]{../fig/SA4/Corlett-Bishop.png}
%DIFDELCMD < \caption{Modified Corlett-Bishop Discomfort Scale mapping used for this evaluation, limited to areas below the knee}\label{fig:SA4-CB}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \hypertarget{fig:SA4-CB}{%
\centering
...
\DIFaddbegin \DIFadd{These prominences were set by observation of the collected data.
}\DIFaddend The heel position at each peak was noted as the maximum heel-rise magnitude for each heel-rise in the set.

Heel-off \DIFdelbegin \DIFdel{initiation }\DIFdelend time was identified \DIFaddbegin \DIFadd{for both sides }\DIFaddend by first inverting the heel-position signal and then identifying the signal peaks with the identical parameters for each side as for the maximum heel-rise magnitude analysis.
...
The point at which the velocity signal crossed 1 cm/s was identified as the heel-off \DIFdelbegin \DIFdel{initiation }\DIFdelend time for that heel-rise.

The FSR voltage was recorded at each heel-off \DIFdelbegin \DIFdel{initiation time }\DIFdelend \DIFaddbegin \DIFadd{time for the right side}\DIFaddend .
...
Then, Python's \texttt{scipy.signal.find\_peaks} algorithm was used to identify peaks with a prominence of \DIFaddbegin \DIFadd{at least }\DIFaddend 0.5 V and a width of 5 samples.
...
\DIFaddbegin \DIFadd{Bayesian linear regressions were modeled was used to analyze the following metrics across all four shoe conditions:
}

\begin{itemize}
\tightlist
\item
  \DIFadd{heel performance score
}\item
  \DIFadd{rating of perceived exertion
}\item
  \DIFadd{left/right difference in heel-rise magnitude
}\item
  \DIFadd{left/right difference in heel-off time
}\item
  \DIFadd{FSR voltage at heel-off
}\item
  \DIFadd{FSR voltage at heel-rise
}\end{itemize}

\DIFadd{Mixed-effects linear models were used to estimate these metrics as they are affected by the various boots (fixed effects); subject specific intercepts (random effects) allows for estimation of the group-level effect while controlling for subject-specific effects.
...
\DIFadd{metric = \beta_{\text{shoe}}x_{i}+\alpha_{subject}^{n}+\sigma
}}\DIFadd{}\end{equation}
\DIFadd{where \(\beta_{\text{shoe}}\) is the effect of each footwear (\(x_{i}\)) on the metric, \(\alpha_{O}\) are the \(n\) subject-specific intercepts, and \(\sigma\) is the unexplained error of the model.
Since the shoe types are categorical, \(\beta_{\text{shoe}}x_{i}\) can be expanded:
}\begin{equation}{
\DIFadd{metric = \beta_{\text{Novel Spaceboot}}+\beta_{\text{Laced Workboot}}+\beta_{\text{Classic Spaceboot}}+\beta_{\text{Unlaced Workboot}}+\alpha_{subject}^{n}+\sigma
}}\DIFadd{}\end{equation}

\DIFadd{The model for each of the metric therefore consisted of an estimate of the effect of each footwear and subject specific intercepts for each subject.
...
The Bambi Python package \mbox{%DIFAUXCMD
\citep{Capretto2020} }\hspace{0pt}%DIFAUXCMD
was used for this analysis.
}

\DIFadd{Bayesian inference starts with the selection of a prior for each of the predictors.
...
\DIFadd{There exists no previous work which can directly inform the prior for heel-rise difference and the differences in heel-off time, as affected by various footwear.
...
Previous work has shown that barefoot humans can lift their heels an average of 12 cm \mbox{%DIFAUXCMD
\citep{Brorsson2017}}\hspace{0pt}%DIFAUXCMD
.
...
\DIFadd{Priors for the subject specific intercepts and unexplained error were left for Bambi to automatically select \mbox{%DIFAUXCMD
\citep{Capretto2020}}\hspace{0pt}%DIFAUXCMD
.
Once priors are chosen and the model is specified, the model is fit using a Markov Chain Monte Carlo (MCMC) method to sample from the posterior, as impelemnted in the PyMC3 Python library \mbox{%DIFAUXCMD
\citep{Salvatier2016}}\hspace{0pt}%DIFAUXCMD
.
...
\DIFadd{The maximal posterior estimate and 95\% Bayesian credible interval were calculated from the sampled posterior distribution for each parameter in each model.
...
\DIFaddend \hypertarget{results-3}{%
\section{Results}\label{results-3}}

\DIFaddbegin \DIFadd{Four out of the five subjects ranked the shoe conditions from best to worst as Laced Workboot, Novel Spaceboot, Unlaced Workboot, Classic Spaceboot.
...
\caption{Mean and maximum of reported Corlett-Bishop discomfort ratings on both the left (control) and right (test) shoe. Results are presented across all subjects.}\label{fig:SA4-CBresults}
}
\end{figure}

\DIFadd{The maximal posterior estimate of the heel performance rating for the novel spaceboot was 6.8, compared to 8.4 for the laced workboot, 2.3 for the classic spaceboot, and 2.4 for the unlaced spaceboot (fig.~\ref{fig:SA4-HPE}).
...
\caption{Estimate (black) of the heel performance rating across all boot conditions, as compared to the subjects' data. The subjects' data is offset along the x-axis to show all data points. Each boot's posterior distributions' overlap as compared to the novel spaceboot are annotated.}\label{fig:SA4-HPE}
}
\end{figure}

\DIFadd{The maximal posterior estimate of RPE for the novel spaceboot was 12.0, compared to 8.8 for the laced workboot, 14.3 for the classic spaceboot, and 8.4 for the unlaced workboot (fig.~\ref{fig:SA4-RPE}).
...
\caption{Estimate (black) of the rating of perceived exertion across all boot conditions, as compared to the subjects' data. The subjects' data is offset along the x-axis to show all data points. Each boot's posterior distributions' overlap as compared to the novel spaceboot are annotated.}\label{fig:SA4-RPE}
}
\end{figure}

\DIFadd{Figure \ref{fig:SA4-exSet} shows an example FSR response and heel-rise measurement from a subject's heel-rise set.
}

\begin{figure}
\hypertarget{fig:SA4-exSet}{%
\centering
\includegraphics[width=0.8\textwidth,height=\textheight]{../fig/SA4/exSet.png}
\caption{Example FSR and heel-rise data trace from a heel-rise set. Detected peaks and heel-off times are shown with the vertical lines}\label{fig:SA4-exSet}
}
\end{figure}

\DIFadd{The maximal posterior estimate of the difference in heel-rise magnitude was 3.3 cm for the novel spaceboot, compared to 3.8 cm for the laced workboot, 5.5 cm for the classic spaceboot, and 4.8 cm for the unlaced workboot (fig.~\ref{fig:SA4-HLD}).
...
\caption{Estimate (black) of the difference in heel-rise magnitude across all boot conditions, as compared to the subjects' data. heel-rise magnitude difference is calculated by subtracting the magnitude of the right foot from the left foot. The subjects' data is offset along the x-axis to show all data points. Each boot's posterior distributions' overlap as compared to the novel spaceboot are annotated.}\label{fig:SA4-HLD}
}
\end{figure}

\DIFadd{The maximal posterior estimate of the difference in heel-off time was 0.0s for the novel spaceboot, compared to -0.3 s for the laced workboot, -0.5 s for the classic spaceboot, and -0.3 s for the unlaced workboot (fig.~\ref{fig:SA4-HOT}).
...
\caption{Estimate (black) of the difference in heel-off time across all boot conditions, as compared to the subjects' data. The difference in heel-off time is calculated as the left foot's (control) heel-off time minus the right foot's (test) heel-off time. Therefore, negative numbers indicate that the right foot lifted after the left foot. The subjects' data is offset along the x-axis to show all data points. Each boot's posterior distributions' overlap as compared to the novel spaceboot are annotated.}\label{fig:SA4-HOT}
}
\end{figure}

\DIFadd{The maximal posterior estimate of the FSR voltage at maximum heel-rise was 2.2 V for the novel spaceboot, compared to 3.3 V for the laced workboot, the classic spaceboot, and the unlaced workboot (fig.~\ref{fig:SA4-FSRmax}).
...
\caption{Estimate (black) of the FSR voltage at maximum heel-rise across all boot conditions, as compared to the subjects' data. The subjects' data is offset along the x-axis to show all data points. Each boot's posterior distributions' overlap as compared to the novel spaceboot are annotated.}\label{fig:SA4-FSRmax}
}
\end{figure}

\DIFadd{The maximal posterior estimate of the FSR voltage at heel-off for the novel spaceboot was 1.6 V, compared to 1.0 V for the laced workboot, 3.0 V for the classic spaceboot, and 2.3 V for the unlaced spaceboot (fig.~\ref{fig:SA4-FSRH0}).
...
\caption{Estimate (black) of the FSR voltage at heel-off across all boot conditions, as compared to the subjects' data. The subjects' data is offset along the x-axis to show all data points. Each boot's posterior distributions' overlap as compared to the novel spaceboot are annotated.}\label{fig:SA4-FSRH0}
}
\end{figure}
...
\DIFaddbegin \DIFadd{This study aimed to evaluate the performance of the novel spacesuit boot design against a laced workboot, unlaced workboot, and classic spacesuit boot design.
...
\DIFadd{Select metrics from this study were further analyzed using Bayesian inference for linear mixed models to objectively assess the boots' performance.
...
\DIFadd{Comfort and exertion are key metrics in assessing the performance of a spacesuit component \mbox{%DIFAUXCMD
\citep{Abercromby2012, Chappell2017}}\hspace{0pt}%DIFAUXCMD
.
The pressurized spacesuit boots had higher levels of reported discomfort compared to the unpressurized work boots (}{[}\DIFadd{f@fig:SA4-CBresults}{]}\DIFadd{).
...
\DIFadd{The FSR voltage metrics provide insight into heel-contact at heel-off and at maximum heel-rise.
...
This may also work in tandem with the designed dynamic lacing aspect of the novel spaceboot (see sec.~\ref{sec:SA3}), which lengthens the lace path of the BOA by relatively moving the tubing from the dial to the guides and requires the guides to come closer together to compensate.
...
\caption{Video frame of heel-rise by subject FT in the novel spaceboot. As the BOA lace length cannot change, the sole is kept at a constant distance to the instep throughout the heel-rise motion. This ensures that the heel is continuously making contact with the}\label{fig:SA4-heelliftcap}
}
\end{figure}

\DIFadd{The heel-off time difference and heel-raise magnitude difference further add to the heel performance characterization by comparing the test boots against the subjects' own control shoes when performing a heel raise.
...
\DIFadd{Subjects rated the novel spacesuit boot's heel performance as below that of the laced workboot, but above the classic spacesuit and unlaced workboot.
...
\DIFadd{However, it is important to note that no quantitative measurements were collected during the short walk exercise, and the laced workboot may have performed better here to align with the subjective ratings of heel performance.
}

\DIFaddend \hypertarget{summary-6}{%
\section{Summary}\label{summary-6}}

\DIFdelbegin %DIFDELCMD < \hypertarget{summary-and-execution-plan}{%
%DIFDELCMD < \chapter{Summary and Execution Plan}\label{summary-and-execution-plan}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{This evaluation compared the novel spacesuit boot against a laced workboot, classic spaceboot, and unlaced workboot.
...
\DIFaddbegin \DIFadd{Pressurization appears to have had a greater impact on comfort than originally anticipated.
...
\DIFadd{Results from this evaluation show that the novel spacesuit boot design is a clear improvement over the classic spacesuit boot design.
...
\DIFaddend Through many advancements of planetary EVA spacesuit design, operator-spacesuit coordination is still not perfectly matched.
...
This thesis \DIFdelbegin \DIFdel{aims }\DIFdelend \DIFaddbegin \DIFadd{was designed }\DIFaddend to determine the feasibility of using dynamic body shape models to improve spacesuit component fit and mobility, specifically with planetary spacesuit boots.
...
\DIFdelbegin %DIFDELCMD < \hypertarget{publications}{%
%DIFDELCMD < \section{Publications}\label{publications}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{This thesis investigated the hypothesis through four specific aims, resulting in novel contributions to the field.
}\DIFaddend 

\DIFdelbegin \DIFdel{Table \ref{tbl:pubs} outlines the peer-reviewed conference and journal papers resulting from this thesis work.
...
}\DIFdelend \DIFaddbegin \DIFadd{The thesis first investigated the ability to quantify heel-lift magnitude inside the spacesuit.
...
%DIFDELCMD < \hypertarget{tbl:pubs}{}
%DIFDELCMD < \begin{longtable}[]{@{}
%DIFDELCMD <   >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.10}}
%DIFDELCMD <   >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.40}}
%DIFDELCMD <   >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.30}}
%DIFDELCMD <   >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.20}}@{}}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{tbl:pubs}%%%
\DIFdel{Peer-reviewed publications}}%DIFAUXCMD
%DIFDELCMD < \tabularnewline
%DIFDELCMD < \toprule
%DIFDELCMD < %%%
\DIFdel{Type }%DIFDELCMD < & %%%
\DIFdel{Title }%DIFDELCMD < & %%%
\DIFdel{Journal }%DIFDELCMD < & %%%
...
%DIFDELCMD < \endhead
%DIFDELCMD < %%%
\DIFdel{Technical Note }%DIFDELCMD < & %%%
\DIFdel{Detecting Heel-Lift in Spacesuit Gait }%DIFDELCMD < & %%%
\DIFdel{Aerospace Medicine and Human Performance }%DIFDELCMD < & %%%
\DIFdel{In Prep.
, expected Nov 2020 }%DIFDELCMD < \\
%DIFDELCMD < & & & \\
%DIFDELCMD < %%%
\DIFdel{Journal Paper }%DIFDELCMD < & %%%
\DIFdel{DynaMo: Dynamic Body Shape and Motion Capture with Intel RealSense Cameras }%DIFDELCMD < & %%%
\DIFdel{Journal of Open Source Software }%DIFDELCMD < & %%%
\DIFdel{Published }%DIFDELCMD < \\
%DIFDELCMD < & & & \\
%DIFDELCMD < %%%
\DIFdel{Journal Paper }%DIFDELCMD < & %%%
\DIFdel{Dynamic foot morphology explained through 4D scanning }\DIFdelend \DIFaddbegin \DIFadd{Following this study, the dynamic morphology of the foot was investigated with a focus on discovering changes which can improve spacesuit boot design.
...
A further contribution includes an analysis on dynamic instep height, including a new method to measure instep height without a plantar surface, }\DIFaddend and \DIFdelbegin \DIFdel{shape modeling }%DIFDELCMD < & %%%
\DIFdel{Journal of Biomechanics }%DIFDELCMD < & %%%
\DIFdel{Under Review }%DIFDELCMD < \\
%DIFDELCMD < & & & \\
%DIFDELCMD < %%%
\DIFdel{Conference Paper }%DIFDELCMD < & %%%
\DIFdel{A Biomechanical Design Framework to Improve Spacesuit Boot Fit }%DIFDELCMD < & %%%
\DIFdel{50th International Conference on Environmental Systems }%DIFDELCMD < & %%%
\DIFdel{Published }%DIFDELCMD < \\
%DIFDELCMD < & & & \\
%DIFDELCMD < %%%
\DIFdel{Journal Paper }%DIFDELCMD < & %%%
\DIFdel{Static and Dynamic Distribution of Instep Height }%DIFDELCMD < & %%%
\DIFdel{Footwear Science }%DIFDELCMD < & %%%
\DIFdel{Proposed, expected Mar 2021 }%DIFDELCMD < \\
%DIFDELCMD < & & & \\
%DIFDELCMD < %%%
\DIFdel{Journal Paper }%DIFDELCMD < & %%%
\DIFdel{Design of A Novel Planetary Spacesuit Boot Design }%DIFDELCMD < & %%%
\DIFdel{Acta Astronautica }%DIFDELCMD < & %%%
\DIFdel{Proposed, expected Aug 2021 }%DIFDELCMD < \\
%DIFDELCMD < & & & \\
%DIFDELCMD < %%%
\DIFdel{Journal Paper }%DIFDELCMD < & %%%
\DIFdel{Comfort and Mobility Evaluation of a Novel Planetary Spacesuit Boot Design }%DIFDELCMD < & %%%
\DIFdel{Aerospace Medicine and Human Performance }%DIFDELCMD < & %%%
\DIFdel{Proposed, expected Feb 2022 }%DIFDELCMD < \\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{longtable}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{the finding that dynamic instep height is subject specific and cannot be accurately predicted from static anthropometries.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \pagebreak
%DIFDELCMD < 

%DIFDELCMD < \hypertarget{tbl:conf}{}
%DIFDELCMD < \begin{longtable}[]{@{}
%DIFDELCMD <   >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.12}}
%DIFDELCMD <   >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.50}}
%DIFDELCMD <   >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.25}}
%DIFDELCMD <   >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.12}}@{}}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{tbl:conf}%%%
\DIFdel{Conference Presentations and Posters}}%DIFAUXCMD
%DIFDELCMD < \tabularnewline
%DIFDELCMD < \toprule
%DIFDELCMD < %%%
\DIFdel{Type }%DIFDELCMD < & %%%
...
%DIFDELCMD < \endhead
%DIFDELCMD < %%%
\DIFdel{Talk }%DIFDELCMD < & %%%
\DIFdel{Using }\DIFdelend \DIFaddbegin \DIFadd{The dynamic foot morphology model was translated into footwear design by first creating a new footwear design framework.
This framework contribution tied both traditional footwear measurements and novel footwear measurements, like the }\DIFaddend dynamic foot morphology \DIFdelbegin \DIFdel{data to design spacesuit footwear }%DIFDELCMD < & %%%
\DIFdel{Footwear Biomechanics Symposium }%DIFDELCMD < & %%%
\DIFdel{July 2019 }%DIFDELCMD < \\
%DIFDELCMD < & & & \\
%DIFDELCMD < %%%
\DIFdel{Talk }%DIFDELCMD < & %%%
\DIFdel{Development of a Dynamic 3D Scanning System with Multiple Intel RealSense Depth Cameras }%DIFDELCMD < & %%%
\DIFdel{International Society of Biomechanics Congress }%DIFDELCMD < & %%%
\DIFdel{Aug 2019 }%DIFDELCMD < \\
%DIFDELCMD < & & & \\
%DIFDELCMD < %%%
\DIFdel{Poster }%DIFDELCMD < & %%%
\DIFdel{Quantifying the Heel-Lift during Spacesuit Gait }%DIFDELCMD < & %%%
\DIFdel{NASA HRP IWS }%DIFDELCMD < & %%%
\DIFdel{Jan 2020 }%DIFDELCMD < \\
%DIFDELCMD < & & & \\
%DIFDELCMD < %%%
\DIFdel{TBD }%DIFDELCMD < & %%%
\DIFdel{Dynamic Body-Shape Models to Reduce Risk OF EVA Spacesuit Injury }%DIFDELCMD < & %%%
\DIFdel{NASA HRP IWS }%DIFDELCMD < & %%%
\DIFdel{Feb 2021 }%DIFDELCMD < \\
%DIFDELCMD < & & & \\
%DIFDELCMD < %%%
\DIFdel{TBD }%DIFDELCMD < & %%%
\DIFdel{Novel Spacesuit Boot Design Developed from Dynamic Foot Shape Modeling }%DIFDELCMD < & %%%
\DIFdel{Footwear Biomechanics Symposium }%DIFDELCMD < & %%%
\DIFdel{July 2021 }%DIFDELCMD < \\
%DIFDELCMD < & & & \\
%DIFDELCMD < %%%
\DIFdel{TBD }%DIFDELCMD < & %%%
\DIFdel{Spacesuit Boot with Improved Comfort and Mobility Developed from Dynamic Shape Modeling }%DIFDELCMD < & %%%
\DIFdel{NASA HRP IWS }%DIFDELCMD < & %%%
\DIFdel{Jan 2022 }%DIFDELCMD < \\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{longtable}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{model, into footwear design variables.
...
\DIFdelbegin %DIFDELCMD < \hypertarget{academic-requirements}{%
%DIFDELCMD < \section{Academic Requirements}\label{academic-requirements}}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{All required coursework was completed as of }\DIFdelend \DIFaddbegin \DIFadd{The final contribution of this thesis lies in the evaluation of the novel spacesuit boot against the classic spacesuit boot, a laced workboot, and an unlaced workboot for fit and comfort.
...
\DIFadd{The performance of the novel spacesuit boot design shows the utility of using body shape models to inform spacesuit component design.
...
As }\DIFaddend the \DIFdelbegin \DIFdel{Spring 2020 semester.
Of the 36 required credits, 30 were taken in ASEN, with 12 at the 6000 level.
MCEN 5228 (Modeling Human Movement) and APPM 5590 (Statistical Modeling) were taken outside of ASEN.
The 6 required math credits were exceeded with taking ASEN 5519 (Experimental Design and Statistical Analysis), APPM 5590 (Statistical Modeling), and ASEN 5044 (Statistical Estimation for Dynamical Systems).
As of the Fall 2020 semester, 15 out of the 30 required Doctoral dissertation credits have been taken.
The remaining 15 credits will be evenly taken during the Spring 2021, Fall 2021, and Spring 2022 semesters.
The teaching practicum has been fulfilled through the mentoring of UROP students in the Summer 2018, Fall 2019, and Spring 2020 semesters.
A TA position is also expected in Fall 2021.
}\DIFdelend \DIFaddbegin \DIFadd{evaluation shows, these two additions to the spacesuit boot design resulted in a much greater improvement in heel performance and reduction of exertion.
...
\DIFdelbegin %DIFDELCMD < \hypertarget{timeline}{%
%DIFDELCMD < \section{Timeline}\label{timeline}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{future-work}{%
\section{Future Work}\label{future-work}}
\DIFaddend 

\DIFdelbegin \DIFdel{Gantt chart \ref{fig:Gantt} shows the timeline for each specific aim of the thesis and for PhD graduation milestones.
}\DIFdelend \DIFaddbegin \DIFadd{This work's effort in translating a foot shape model to spacesuit boot design can serve as a template for other spacesuit components as well.
...
%DIFDELCMD < \hypertarget{fig:Gantt}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=1\textwidth,height=\textheight]{../fig/Gantt.png}
%DIFDELCMD < \caption{Gantt chart showing PhD timeline}\label{fig:Gantt}
%DIFDELCMD < }
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Similarly, this thesis' work is not limited to spacesuit design.
This process can be applied to apparel or terrestrial footwear for any application where injury might be a risk.
}

\DIFadd{Future advancements in technology can also work to improve the fidelity of the foot shape model.
...
\DIFadd{Finally, this thesis aims to show the importance in understanding all dimensions of human motion when it comes to spacesuit design.
